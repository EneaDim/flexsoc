...
| Clock/reset domains | `N_CLOCKS`, `CLOCK_DOMAINS`, `CLOCK_RELATIONSHIPS` | RTL scaffold assumptions, CDC/RDC, formal mode, SDC, simulation clocks |
...
## 10. Canonical timing intent: why the SDC is not read by Yosys synthesis

FlexSoC intentionally separates **technology mapping** from **timing constraint/sign-off**. Timing intent is created once from the canonical clock model, but it is translated into two different representations because Yosys/ABC and OpenSTA/OpenROAD solve different problems.

```text
FlexSoC ClockConfig
  ├─ synthesis hints
  │    ├─ target Liberty
  │    ├─ fastest clock period where ABC needs a delay target
  │    └─ abc.constr: driving cell + output load
  │
  └─ canonical SDC
       ├─ pre-implementation OpenSTA
       ├─ ORFS/OpenROAD implementation
       └─ 6_final.sdc → post-implementation OpenSTA
```

The key fact is that **the ASIC Yosys scripts generated by current FlexSoC do not read the SDC**. This is deliberate architecture, not a missing command.

### 10.1 Source of truth: the FlexSoC clock model

The canonical configuration is built by `clock_config()` from:

- `N_CLOCKS`;
- `CLOCK_DOMAINS`;
- `CLOCK_RELATIONSHIPS`;
- `CLK_PERIOD` for the single-clock/default case;
- per-domain name, clock signal, reset, period, and reset polarity.

Conceptual multi-clock example:
...
The same `ClockConfig` feeds CDC/RDC, SDC generation and synthesis context. Resets belong to the canonical model because CDC/RDC and other backends need them; **the current SDC generator does not invent reset exceptions or recovery/removal policies that were not explicitly modeled**.

### 10.2 What synthesis actually receives

`syn/syn.py` does not emit `read_sdc` into `synth.ys` or `synth_sv.ys`. ASIC synthesis instead receives a deliberately small subset of timing intent:
...
- `delay` strategy passes ABC `-D <fastest_period_ps>` plus `-constr abc.constr`;
- `area` uses `area.abc` plus `abc.constr` without turning the SDC into a synthesis timing graph;
- `none` uses direct ABC mapping with `-D <fastest_period_ps>`;
- `abc.constr` contains only `set_driving_cell` and `set_load`.

The Python `SynthesisConfig` still has an `sdcdir` field, but **the current ASIC renderer does not consume it, and its presence must not be interpreted as Yosys reading an SDC**.

### 10.3 Why this separation is deliberate

FlexSoC uses Yosys/ABC to **optimize and map logic**, not as the sign-off timing engine. The mapper therefore receives enough timing hints to make useful cell choices, while complete timing semantics are delegated to OpenSTA/OpenROAD.

This avoids four problems:

- **duplicated timing semantics** — generated clocks, asynchronous groups, I/O delays and exceptions should have one authoritative interpretation;
- **false pre-layout precision** — before placement, CTS and routing there is no routed parasitic model or real clock insertion delay;
- **tool drift** — a partial synthesis-side SDC translation could diverge from what OpenSTA/OpenROAD actually consume;
- **unnecessary coupling** — synthesis should produce a good mapped netlist, while physical timing repair, CTS and timing-driven routing belong to implementation.

This is a choice of the **current FlexSoC flow**, not a statement that Yosys can never participate in flows with richer timing constraints.

### 10.4 `abc.constr` is not an SDC

The `-constr` format of the Yosys ABC pass is intentionally minimal:
...
It approximately tells the mapper:

- how strong the external primary-input driver is;
- what load each primary output sees.
...
- clock-related input/output delays;
- false or multicycle paths;
- propagated clock latency;
- parasitics.

Those concepts belong to SDC/STA and, later, the physical timing model.

### 10.5 How FlexSoC automatically generates the canonical SDC

`signoff/sta.py` owns the canonical SDC renderer, exposed through `signoff.pre.setup_sdc()`. Conceptually:
...
For a **single-clock** design the file contains at least:
...
The `0.2` comes from the default `SDC_IO_DELAY_PCT=0.2`: it is a configurable integration assumption, not a universal physical property of the chip.

For a **multi-clock** design FlexSoC generates, according to the declared relationships:

- `create_clock` for primary clocks;
- `create_generated_clock` for `generated` relationships;
- `set_clock_groups -asynchronous` for `async` relationships;
- no guessed multi-clock I/O delays: port-to-clock association is integration-specific and must be declared explicitly when that capability is extended.

A `sync` relationship is currently recorded without automatically adding a timing exception.

### 10.6 Where the SDC enters the real flow

Once generated, the SDC becomes the authoritative pre-layout timing contract:
...
ORFS therefore **does not resynthesize** the design: it receives the FlexSoC mapped netlist and canonical SDC separately. From that point OpenROAD can use timing intent during timing-driven placement, timing repair, CTS and routing.

At the end of implementation ORFS also produces:
...
Post-implementation sign-off does not fall back to the old pre-layout model. FlexSoC consumes **`6_final.sdc` + final netlist + SPEF**, propagates clocks and measures the actual routed design.
...
In other words: **clock intent is shared; the SDC file is not, because every stage does not need the same representation**. Synthesis uses mapping hints; STA and implementation use the complete timing contract.
...
- fastest clock period as an ABC optimization hint where applicable, **not the SDC**;
...
Post-implementation STA changes the analysis model from ideal/pre-layout to physical/routed:
...
Additional routed sections include:

- SPEF annotation coverage / unannotated nets;
- clock latency;
- setup and hold clock skew;
- worst routed paths even when timing is met;
- full expanded clock/path reporting with slew, capacitance, net and fanout fields.

This is the key quantitative distinction from post-synthesis STA: cell delay, routed wire delay and physical clock-tree behavior are now analyzed together.
