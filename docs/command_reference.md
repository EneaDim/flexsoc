...
To accept an intentional generated-file edit, use `fx validate_override --set STAGE=<setup_target>`. Validation never accepts stale lineage, and any later source/config/parent change requires a new setup or validation. Restoring the exact canonical generated bytes returns the stage to `CLEAN` automatically; rerunning the setup target also regenerates the canonical collateral and resets any validated override.
...
| `fx validate_override` | Accept modified machine-generated setup collateral for the current input/parent lineage. | `STAGE=<setup_target>` | Only `MODIFIED` collateral can be accepted; `STALE`/`INVALID` states require regeneration or repair. |
