...
The recovery action depends on the state:

```text
MODIFIED  → generated bytes were edited manually; validate_override may accept that exact edit
STALE     → source/config/parent lineage changed; rerun the setup target with the intended settings
INVALID   → required inputs/artifacts/provenance are missing; repair them and rerun setup
```

A configuration experiment is therefore not an override workflow. For one command, `fx syn --set TARGET_OPT=delay1` lets automatic setup and synthesis share the same effective configuration. For a multi-command flow, persist the choice first with `fx settings TARGET_OPT=delay1`, then run setup and consumers.
