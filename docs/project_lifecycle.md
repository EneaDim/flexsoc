...
| Clock/reset topology | `N_CLOCKS`, `CLOCK_DOMAINS`, `CLOCK_RELATIONSHIPS` bootstrap metadata | first SDC scaffold, reset ownership/polarity, reset-aware backend context |
| Expected functional behavior | authored model + scenarios | vectors consumed by simulators |
| Timing intent | authored `constraints/design.sdc` | functional clock stimulus, CDC clock relationships, synthesis drive/load collateral, STA, implementation, SDF/GLS timing context |
...
synthesis, EQY, STA, SDF, power, and implementation scripts
...
fx validate_override --set STAGE=<setup_target>
    ↓
run consumer → runs as VALIDATED_OVERRIDE
    ↓
restore canonical bytes → CLEAN
```

The validation applies only to the exact effective hash and current parent/input lineage. A different edit is not implicitly trusted. `validate_override` is only for `MODIFIED` generated bytes; source/config/parent changes make the setup `STALE`, which must be regenerated instead.

Configuration changes never trigger implicit regeneration. Persist the intended configuration first (`fx settings TARGET_OPT=delay1`), regenerate the affected setup explicitly (`fx setup_syn --force`), then run `fx syn`. A run command with a different one-shot configuration sees the existing setup as STALE and refuses to continue.

Execution stages must treat setup collateral as immutable: runtime/scenario copies may be generated, but a run must not rewrite the canonical files tracked by provenance. The run also snapshots design intent separately from qualification outputs: common intent lives in `meta/design_intent.json`, while effective technology settings live in `meta/<pdk>/settings.json` for every qualified PDK. Lint and CDC/RDC are retained as first-class qualification evidence, not just summarized status: package paths preserve `logs/lint/`, normalized `analysis/cdc_rdc/*.json`, and `logs/analysis/cdc_rdc/cdc_rdc.log`.
...
FlexSoC distinguishes authored initialization (`init_*`), generated execution setup (`setup_*`), execution (`run_*`), and evidence collection (`collect_*`). Inspection and diagnostics use `show_*` and `debug_*`. Legacy backend method names remain compatibility aliases; no lifecycle semantics are hidden behind the rename.
...
## 5. Bootstrap one or many clock/reset domains

FlexSoC starts from a compact domain/reset description so a new IP can be scaffolded before an authored SDC exists. This bootstrap metadata is not meant to remain a second timing source of truth.
...
Domain encoding is `name:clock_signal:reset_signal:period_ns:reset_polarity`; relationships are explicit `async`, `sync`, or `generated` declarations. FlexSoC never guesses asynchronous relationships.

These settings initialize the first timing scaffold and preserve reset ownership/polarity for reset-aware flows. After `constraints/design.sdc` is created, clock period/waveform, clock relationships, latency, uncertainty, I/O timing, drive/load and timing exceptions are authored there.
...
### 7.3 Establish the authored timing contract

After RTL structure and port names are stable enough to lint cleanly, initialize the SDC once:

```bash
fx sdc --force
```

Then review and edit `constraints/design.sdc`. It is the single timing source of truth for the rest of the lifecycle. The scaffold is intentionally readable and complete enough to expose the decisions a production IP must eventually own:

- primary and generated clocks;
- clock waveform, source latency, uncertainty and transition;
- explicit asynchronous clock groups;
- input/output min/max delay;
- input drive or driving cell;
- output load;
- commented false-path and multicycle sections;
- optional design-rule limits.

Functional SV and cocotb clocks use the SDC period/waveform/source latency directly. `set_clock_uncertainty` becomes bounded reproducible functional jitter (`±max(setup, hold)`, 1 ps resolution) driven by the existing run `SEED`; it remains the same authored uncertainty seen by STA. `set_clock_transition` is not turned into an analog simulation slew.

**Advance when:** clocks and relationships match the architecture, interface timing assumptions are reviewed, and exceptions are either absent or have an explicit architectural reason.

### 7.4 CDC and RDC analysis

Generate the structural extraction explicitly, then run the checker:
...
FlexSoC builds a technology-independent sequential dependency graph and runs one fixed checker order: scalar/multi-bit CDC crossings, FIFO/Gray candidates, handshakes, reconvergence, setup/glitch, reset-domain crossings, reset synchronizers, reset release, and reset sequencing. Clock relationships come from `design.sdc`; reset signal ownership and polarity remain the minimal bootstrap metadata that SDC does not express.

The result distinguishes structurally safe patterns from warnings, structural errors, and `REVIEW` obligations that need temporal/protocol verification. The compact terminal output always points to the detailed log; `--live` streams the full analysis.

**Advance when:** structural `ERROR` findings are repaired and every remaining `REVIEW`/`WARN` has an explicit closure plan.

### 7.5 Property formal verification

Create authored starter properties once and generate the execution collateral explicitly:
...
Formal combines generated CSR semantics with authored design assertions and covers. It is the right gate for invariants, protocol safety, legal state transitions, FIFO safety, reset convergence, and other properties that should not depend on selecting a particular simulation scenario.

**Advance when:** real counterexamples are fixed, expected covers are reachable, and proof assumptions/limits are reviewed rather than used to hide DUT behavior.

### 7.6 Functional regression and coverage

Functional DV closes representative end-to-end behavior using the authored model/scenarios and shared vectors across the supported simulation backends:

```bash
fx setup_model --force          # first scaffold only
fx tests_gen --force
...
The SV and cocotb harnesses consume the same `design.sdc` clock model and the same run `SEED`, so non-50% duty cycle, source phase/latency, and bounded clock jitter are reproducible across backends. Vector input/output scheduling remains independent of the clock-noise model: expected outputs may be checked before later input events without forcing all input vectors to complete first.

**Advance when:** the required scenario catalogue passes on the configured backends, coverage gaps are reviewed, and failures are reproducible from a named test, seed, log, and waveform.

### 7.7 Verification closure

The verification stage is complete only when lint, timing intent, CDC/RDC, formal, and functional evidence are mutually consistent. Any RTL, CSR, clock/reset, model, property, interface or SDC change reopens the gates whose assumptions changed.

---

## 8. Pre-synthesis readiness

By this point `constraints/design.sdc` already exists and has been exercised by CDC/RDC and functional DV. There is no second sign-off SDC generation step. Before selecting a technology, review the technology-independent evidence together:

```text
RTL/filelists clean
+ authored design.sdc reviewed
+ CDC/RDC closed
+ formal closed
+ functional regression/coverage reviewed
```

Constraints are assumptions. An unconstrained path is not a passing path, and a false/multicycle exception is an architectural statement rather than a timing-fix switch. Keep exceptions narrow and owned.

When the selected PDK is activated, regenerate only the technology-dependent setup that consumes this common intent:
...
`setup_syn` derives the ABC-compatible input-drive/output-load collateral from `design.sdc`. `setup_signoff` generates OpenSTA scripts; it does not create or rewrite timing intent.

> **When constraints go wrong:** fix the authored SDC when the timing assumption is wrong. If the domain/reset topology itself changed, update bootstrap metadata and intentionally regenerate/review the SDC scaffold. Regenerate affected TB, CDC, synthesis, implementation and sign-off setup explicitly; never hide missing clocks or unconstrained endpoints behind broad exceptions.
...
- selected top;
- clock optimization target derived from the authored SDC;
- selected PDK and Liberty data;
- synthesis and optimization strategy;
- `abc.constr` drive/load collateral derived from `design.sdc`;
- required cell models.
...
#### Explicit setup and run-only pipelines

Atomic execution commands do not prepend setup. `syn`, formal execution, `eqy`, sign-off execution, CDC/RDC and PnR consume previously generated collateral and validate its provenance before running. Generated setup is created explicitly once and reused while it remains `CLEAN` or `VALIDATED_OVERRIDE`.

A visible pipeline is therefore the normal pipeline:
...
If owning inputs or configuration change, rerun only the affected setup with `--force`. After each PDK switch, refresh the technology-dependent setup stages and the SV/cocotb drivers required by GLS. Setup never substitutes for earlier result-producing stages: EQY still requires a synthesized netlist, activity power still requires completed GLS evidence, and changing PDK still requires rerunning technology-bound synthesis/sign-off.
...
| stale generated boundary | old wrapper, filelist, testbench, tool setup | regenerate the specific generated scaffold; edit/regenerate `design.sdc` only intentionally |
...
Designer response depends on the owner of the change. For a timing-only change, edit `constraints/design.sdc` and regenerate affected machine-owned setup:

```bash
fx setup_tb setup_cocotb setup_cdc_rdc setup_syn setup_eqy setup_signoff --force
```

If the clock/reset topology itself changed, update `N_CLOCKS`/`CLOCK_DOMAINS`/`CLOCK_RELATIONSHIPS`, intentionally regenerate `fx sdc --force`, reapply/review authored SDC content, then regenerate the same downstream setup. Review CDC/RDC explicitly whenever clock relationships change. An exception is an architectural statement and requires a narrow scope, rationale, and owner.
...
```bash
fx setup_syn --force --live
fx syn --live
fx setup_eqy --force --live
fx eqy --live
fx eqy_debug
fx eqy_debug <partition>
```
...
The package retains reusable synthesis/implementation collateral, EQY/SDC, one
canonical Tcl per sign-off family, selected post-synthesis GLS JSON, compact
coverage summaries, final sign-off `.rpt`/`.json`/`.sdf` evidence, and per-PDK
metadata. Scenario/corner/workload Tcl copies are runtime collateral and are
excluded together with logs, waveforms, converted activity data, hidden transient
sign-off files, RTLIL debug checkpoints, and Python caches.
