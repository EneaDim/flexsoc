...
For a one-shot configuration experiment, let the consumer perform automatic setup with the same effective settings, for example `fx syn --set TARGET_OPT=delay1`. For a longer explicit pipeline, persist the configuration first (`fx settings TARGET_OPT=delay1`), then run `setup_syn` and `syn --no-setup`. This avoids accidentally comparing a setup recorded for one configuration against a later command using another.

Execution stages must treat setup collateral as immutable: runtime/scenario copies may be generated, but a run must not rewrite the canonical files tracked by provenance.
