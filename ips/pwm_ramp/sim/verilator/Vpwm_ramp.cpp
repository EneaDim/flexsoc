...
Vpwm_ramp::Vpwm_ramp(VerilatedContext* _vcontextp__, const char* _vcname__)
    : VerilatedModel{*_vcontextp__}
    , vlSymsp{new Vpwm_ramp__Syms(contextp(), _vcname__, this)}
...
Vpwm_ramp::Vpwm_ramp(const char* _vcname__)
    : Vpwm_ramp(Verilated::threadContextp(), _vcname__)
{
}
...
#endif  // VL_DEBUG
void Vpwm_ramp___024root___eval_static(Vpwm_ramp___024root* vlSelf);
void Vpwm_ramp___024root___eval_initial(Vpwm_ramp___024root* vlSelf);
void Vpwm_ramp___024root___eval_settle(Vpwm_ramp___024root* vlSelf);
void Vpwm_ramp___024root___eval(Vpwm_ramp___024root* vlSelf);

void Vpwm_ramp::eval_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+++++TOP Evaluate Vpwm_ramp::eval_step\n"); );
#ifdef VL_DEBUG
    // Debug assertions
    Vpwm_ramp___024root___eval_debug_assertions(&(vlSymsp->TOP));
#endif  // VL_DEBUG
    vlSymsp->__Vm_deleter.deleteAll();
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) {
        vlSymsp->__Vm_didInit = true;
        VL_DEBUG_IF(VL_DBG_MSGF("+ Initial\n"););
        Vpwm_ramp___024root___eval_static(&(vlSymsp->TOP));
        Vpwm_ramp___024root___eval_initial(&(vlSymsp->TOP));
        Vpwm_ramp___024root___eval_settle(&(vlSymsp->TOP));
    }
    VL_DEBUG_IF(VL_DBG_MSGF("+ Eval\n"););
    Vpwm_ramp___024root___eval(&(vlSymsp->TOP));
    // Evaluate cleanup
...
//============================================================
// Events and timing
bool Vpwm_ramp::eventsPending() { return false; }

uint64_t Vpwm_ramp::nextTimeSlot() {
...
const char* Vpwm_ramp::name() const {
    return vlSymsp->name();
}
...
void Vpwm_ramp___024root___eval_final(Vpwm_ramp___024root* vlSelf);

VL_ATTR_COLD void Vpwm_ramp::final() {
    Vpwm_ramp___024root___eval_final(&(vlSymsp->TOP));
}

//============================================================
// Implementations of abstract methods from VerilatedModel

const char* Vpwm_ramp::hierName() const { return vlSymsp->name(); }
const char* Vpwm_ramp::modelName() const { return "Vpwm_ramp"; }
unsigned Vpwm_ramp::threads() const { return 1; }
void Vpwm_ramp::prepareClone() const { contextp()->prepareClone(); }
void Vpwm_ramp::atClone() const {
