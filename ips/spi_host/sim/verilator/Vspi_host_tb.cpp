...
Vspi_host_tb::Vspi_host_tb(VerilatedContext* _vcontextp__, const char* _vcname__)
    : VerilatedModel{*_vcontextp__}
    , vlSymsp{new Vspi_host_tb__Syms(contextp(), _vcname__, this)}
    ...
    , spi_host_tb{vlSymsp->TOP.spi_host_tb}
...
Vspi_host_tb::Vspi_host_tb(const char* _vcname__)
    : Vspi_host_tb(Verilated::threadContextp(), _vcname__)
{
}
...
#endif  // VL_DEBUG
void Vspi_host_tb___024root___eval_static(Vspi_host_tb___024root* vlSelf);
void Vspi_host_tb___024root___eval_initial(Vspi_host_tb___024root* vlSelf);
void Vspi_host_tb___024root___eval_settle(Vspi_host_tb___024root* vlSelf);
void Vspi_host_tb___024root___eval(Vspi_host_tb___024root* vlSelf);

void Vspi_host_tb::eval_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+++++TOP Evaluate Vspi_host_tb::eval_step\n"); );
#ifdef VL_DEBUG
    // Debug assertions
    Vspi_host_tb___024root___eval_debug_assertions(&(vlSymsp->TOP));
#endif  // VL_DEBUG
    ...
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) {
        ...
        Vspi_host_tb___024root___eval_static(&(vlSymsp->TOP));
        Vspi_host_tb___024root___eval_initial(&(vlSymsp->TOP));
        Vspi_host_tb___024root___eval_settle(&(vlSymsp->TOP));
    }
    VL_DEBUG_IF(VL_DBG_MSGF("+ Eval\n"););
    Vspi_host_tb___024root___eval(&(vlSymsp->TOP));
    // Evaluate cleanup
    Verilated::endOfEval(vlSymsp->__Vm_evalMsgQp);
}

void Vspi_host_tb::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vspi_host_tb::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
//============================================================
// Events and timing
bool Vspi_host_tb::eventsPending() { return !vlSymsp->TOP.__VdlySched.empty(); }

uint64_t Vspi_host_tb::nextTimeSlot() { return vlSymsp->TOP.__VdlySched.nextTimeSlot(); }

//============================================================
// Utilities

const char* Vspi_host_tb::name() const {
    return vlSymsp->name();
}
...
void Vspi_host_tb___024root___eval_final(Vspi_host_tb___024root* vlSelf);

VL_ATTR_COLD void Vspi_host_tb::final() {
    Vspi_host_tb___024root___eval_final(&(vlSymsp->TOP));
}

//============================================================
// Implementations of abstract methods from VerilatedModel

const char* Vspi_host_tb::hierName() const { return vlSymsp->name(); }
const char* Vspi_host_tb::modelName() const { return "Vspi_host_tb"; }
unsigned Vspi_host_tb::threads() const { return 1; }
void Vspi_host_tb::prepareClone() const { contextp()->prepareClone(); }
void Vspi_host_tb::atClone() const {
    contextp()->threadPoolpOnClone();
}
std::unique_ptr<VerilatedTraceConfig> Vspi_host_tb::traceConfig() const {
...
void Vspi_host_tb___024root__trace_decl_types(VerilatedVcd* tracep);

void Vspi_host_tb___024root__trace_init_top(Vspi_host_tb___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vspi_host_tb___024root__trace_decl_types(tracep);
    Vspi_host_tb___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void Vspi_host_tb___024root__trace_register(Vspi_host_tb___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void Vspi_host_tb::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vspi_host_tb::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vspi_host_tb___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
