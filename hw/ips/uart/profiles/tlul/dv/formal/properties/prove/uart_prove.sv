...
module uart_design_prove (
  ...
  input logic rx_i, tx_o, tx_enable, rx_enable, sys_loopback, line_loopback,
  ...
  always_comb begin
    assert (rx_enable);
    assert (tx_o == (line_loopback ? rx_i : tx_out_q));
...
bind uart_core uart_design_prove uart_design_prove_i (
  .clk_i(clk_i), .rst_ni(rst_ni), .rx_i(rx_i), .tx_o(tx_o),
