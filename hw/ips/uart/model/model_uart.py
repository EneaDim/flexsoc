"""Generate UART reference vectors from functional pins and software-visible CSRs.

UART intentionally exercises both model interfaces supported by FlexSoC:
* ``cio_rx_i`` / ``cio_tx_o`` / ``cio_tx_en_o`` are direct functional pins;
* configuration, FIFO traffic, status, and received data use generated CSR objects.

Register layout, reset values, masks, and vector serialization come exclusively
from ``regmap_uart.py``. This file owns only UART behavior and test scenarios.
"""
