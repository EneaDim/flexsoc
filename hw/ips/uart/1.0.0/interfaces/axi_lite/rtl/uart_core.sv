...
module uart_core (
  ...
  // Local reset distribution follows the UART functional islands.
  prim_flop #(
    .Width      (1),
    .ResetValue (1'b0)
  ) u_timing_reset_branch (
    .clk_i (clk_i),
    .rst_ni(rst_ni),
    .d_i   (1'b1),
    .q_o   (timing_rst_ni)
  ...
  always_ff @(posedge clk_i or negedge timing_rst_ni) begin
    if (!timing_rst_ni) begin
      nco_sum_q <= '0;
  ...
  always_ff @(posedge clk_i or negedge tx_rst_ni) begin
    if (!tx_rst_ni) begin
      tx_out_q <= 1'b1;
  ...
  always_ff @(posedge clk_i or negedge rx_rst_ni) begin
    if (!rx_rst_ni) begin
      rx_sync_q1 <= 1'b1;
      rx_sync_q2 <= 1'b1;
    end else begin
