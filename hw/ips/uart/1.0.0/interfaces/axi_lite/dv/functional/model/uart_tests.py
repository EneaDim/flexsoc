...
def ctrl_fields(test: str) -> dict[str, int]:
    """Return the static CTRL configuration used by one UART scenario."""

    ctrl = {"TX": 1, "RX": 1, "NCO": NCO}
    if test in {"smoke", "line_loopback"}:
        ctrl["LLPBK"] = 1
    elif test in {"reconfig", "parity_reconfig"}:
        ctrl["SLPBK"] = 1
    elif test.startswith("random_seed_") or test == "noise_filter":
        ctrl["NF"] = 1
    elif test == "fifo_reset":
        # Retain TX FIFO entries until the test explicitly clears both FIFOs.
        ctrl["TX"] = 0
    elif test == "parity_error":
        ctrl.update(PARITY_EN=1, PARITY_ODD=1)
    return ctrl
...
def fifo_reset_vectors() -> tuple[list[str], list[str]]:
    """Exercise independent FIFO fill state and write-triggered clear actions."""

    data_in = [
        drive(0, "rx_i", 1),
    ]
    rows, cycle = uart_rx_rows(32, 0x6D)
    data_in.extend(rows)
    check_cycle = cycle + 24
    data_in.extend([
        CSR.WDATA.vector_write(check_cycle + 4, WDATA=0x12),
        CSR.WDATA.vector_write(check_cycle + 8, WDATA=0x34),
        CSR.FIFO_CTRL.vector_write(check_cycle + 24, RXRST=1, TXRST=1),
    ])
    data_out = [
        CSR.FIFO_STATUS.vector_read(check_cycle, TXLVL=0, RXLVL=1),
        CSR.FIFO_STATUS.vector_read(check_cycle + 16, TXLVL=2, RXLVL=1),
        CSR.FIFO_STATUS.vector_read(check_cycle + 40, TXLVL=0, RXLVL=0),
        CSR.STATUS.vector_read(
            check_cycle + 40, TXFULL=0, RXFULL=0, TXEMPTY=1, TXIDLE=1, RXEMPTY=1
        ),
    ]
    return data_in, data_out


def noise_filter_vectors() -> tuple[list[str], list[str]]:
    """Reject a one-cycle RX glitch, then accept a stable UART frame."""
    ...
    rows, cycle = uart_rx_rows(96, 0xA6)
    data_in.extend(rows)
    check_cycle = cycle + 48
    data_out = [
        CSR.FIFO_STATUS.vector_read(check_cycle, TXLVL=0, RXLVL=1),
        CSR.RDATA.vector_read(check_cycle, RDATA=0xA6),
        CSR.FIFO_STATUS.vector_read(check_cycle, TXLVL=0, RXLVL=0),
    ]
    return data_in, data_out


def parity_error_vectors() -> tuple[list[str], list[str]]:
    """Reject bad parity while accepting the following correctly formed frame."""

    data_in = [
        drive(0, "rx_i", 1),
    ]
    bad_rows, cycle = uart_rx_rows(32, 0x5A, parity=True, odd=True, bad_parity=True)
    data_in.extend(bad_rows)
    good_rows, cycle = uart_rx_rows(cycle + 32, 0xC3, parity=True, odd=True)
    data_in.extend(good_rows)
    check_cycle = cycle + 48
    data_out = [
        CSR.FIFO_STATUS.vector_read(check_cycle, TXLVL=0, RXLVL=1),
        CSR.RDATA.vector_read(check_cycle, RDATA=0xC3),
        CSR.FIFO_STATUS.vector_read(check_cycle, TXLVL=0, RXLVL=0),
    ]
    return data_in, data_out
...
def vectors(test: str) -> tuple[list[str], list[str]]:
    ...
    scenarios = {
        ...
        "fifo_reset": fifo_reset_vectors,
        "noise_filter": noise_filter_vectors,
        "parity_reconfig": reconfig_vectors,
        "parity_error": parity_error_vectors,
    }
    try:
