...
VlCoroutine Vtop___024root___eval_initial__TOP__Vtiming__0(Vtop___024root* vlSelf) {
    ...
    auto& vlSelfRef = std::ref(*vlSelf).get();
    // Locals
    IData/*31:0*/ uart_tb__DOT____VlemExpr_0;
    // Body
    uart_tb__DOT____VlemExpr_0 = VL_VALUEPLUSARGS_INN(64, "WAVE=%s"s, 
                                                      vlSelfRef.uart_tb__DOT__wave_path);
    if (uart_tb__DOT____VlemExpr_0) {
        ++(vlSelf->__Vcoverage[320]);
    } else {
        vlSelfRef.uart_tb__DOT____VlemExpr_1 = VL_VALUEPLUSARGS_INN(64, "VCD=%s"s, 
                                                                    vlSelfRef.uart_tb__DOT__wave_path);
        if (vlSelfRef.uart_tb__DOT____VlemExpr_1) {
            ++(vlSelf->__Vcoverage[318]);
        } else {
            vlSelfRef.uart_tb__DOT__wave_path = ""s;
            ++(vlSelf->__Vcoverage[317]);
        }
        ++(vlSelf->__Vcoverage[319]);
    }
    if (VL_UNLIKELY(((""s != vlSelfRef.uart_tb__DOT__wave_path)))) {
        VL_WRITEF_NX("[TB] dumpfile = %s owner=wrapper\n",1
                     , 'S',&(vlSelfRef.uart_tb__DOT__wave_path));
        vlSymsp->_vm_contextp__->dumpfile(vlSelfRef.uart_tb__DOT__wave_path);
