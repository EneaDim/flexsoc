    ...
    initial begin
      ...
      if (sdf_path != "") begin
        `ifdef FLEXSOC_SDF_MIN
          $display("[TB] sdf = %s scope=u_rv_timer mode=MINIMUM", sdf_path);
          $sdf_annotate(sdf_path, u_rv_timer);
        `elsif FLEXSOC_SDF_TYP
          $display("[TB] sdf = %s scope=u_rv_timer mode=TYPICAL", sdf_path);
          $sdf_annotate(sdf_path, u_rv_timer);
        `else
          $display("[TB] sdf = %s scope=u_rv_timer mode=MAXIMUM", sdf_path);
          $sdf_annotate(sdf_path, u_rv_timer);
        `endif
      end
    end
