          ...
          initial begin
            ...
            if (sdf_path != "") begin
              `ifdef FLEXSOC_SDF_MIN
                $display("[TB] sdf = %s scope=u_cordic mode=MINIMUM", sdf_path);
                $sdf_annotate(sdf_path, u_cordic);
              `elsif FLEXSOC_SDF_TYP
                $display("[TB] sdf = %s scope=u_cordic mode=TYPICAL", sdf_path);
                $sdf_annotate(sdf_path, u_cordic);
              `else
                $display("[TB] sdf = %s scope=u_cordic mode=MAXIMUM", sdf_path);
                $sdf_annotate(sdf_path, u_cordic);
              `endif
            end
          end
