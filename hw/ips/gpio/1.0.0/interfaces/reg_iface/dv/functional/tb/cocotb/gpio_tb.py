...
async def _flexsoc_clock(signal):
    ...
    jitter_prev_ps = 0
    cocotb.log.info(
        "[FLEXSOC CLOCK] clock=core jitter=uniform bound_ps=25 seed=%d",
        base_seed,
    )
    signal.value = 0
    if 0.1 > 0:
    ...
    while True:
        signal.value = 1
        await Timer(4.95, unit="ns")
        signal.value = 0
        if 25:
            jitter_state = _xorshift32(jitter_state)
            jitter_next_ps = int(jitter_state % 51) - 25
        else:
            jitter_next_ps = 0
        low_delay_ps = 5050 + jitter_next_ps - jitter_prev_ps
        if low_delay_ps <= 0:
...
@cocotb.test()
async def gpio_generated_test(dut):
