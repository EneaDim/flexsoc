...
(
  ...
  // Separate pad-filter state from software-driven GPIO state.
  prim_flop #(
    .Width      (1),
    .ResetValue (1'b0)
  ) u_filter_reset_branch (
    .clk_i (clk_i),
    .rst_ni(rst_ni),
    .d_i   (1'b1),
    .q_o   (filter_rst_ni)
  ...
  always_ff @(posedge clk_i or negedge state_rst_ni) begin
    if (!state_rst_ni) begin
      gpio_o_q <= '0;
