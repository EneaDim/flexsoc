`ifdef VERILATOR
// Auto-generated vector driver for uart_master.
...
task automatic tb_drive_input(input string name, input logic [31:0] value);
  ...
  else if (name == "gnt_i") begin
    gnt_i = value;
    tb_vector_apply_count++;
    $display("[TB][DRV] gnt_i <= 0x%08h", value);
  end
  else if (name == "valid_i") begin
    valid_i = value;
    tb_vector_apply_count++;
    $display("[TB][DRV] valid_i <= 0x%08h", value);
  end
  else if (name == "rdata_i") begin
    rdata_i = value;
    tb_vector_apply_count++;
    $display("[TB][DRV] rdata_i <= 0x%08h", value);
  end
  else if (name == "err_i") begin
    err_i = value;
    tb_vector_apply_count++;
    $display("[TB][DRV] err_i <= 0x%08h", value);
  end
