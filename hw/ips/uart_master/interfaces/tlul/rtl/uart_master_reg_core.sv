...
module uart_master_reg_core (
  input clk_i,
  input rst_ni,
  input  uart_master_reg_pkg::reg_req_t reg_req_i,
  output uart_master_reg_pkg::reg_rsp_t reg_rsp_o,
  // To HW
  output uart_master_reg_pkg::uart_master_reg2hw_t reg2hw, // Write
  input  uart_master_reg_pkg::uart_master_hw2reg_t hw2reg, // Read
  ...
  logic [5:0] addr_hit;
  always_comb begin
    addr_hit[0] = (reg_addr == UART_MASTER_CTRL_OFFSET);
    addr_hit[1] = (reg_addr == UART_MASTER_STATUS_OFFSET);
    addr_hit[2] = (reg_addr == UART_MASTER_RDATA_OFFSET);
    addr_hit[3] = (reg_addr == UART_MASTER_WDATA_OFFSET);
    addr_hit[4] = (reg_addr == UART_MASTER_FIFO_CTRL_OFFSET);
    addr_hit[5] = (reg_addr == UART_MASTER_FIFO_STATUS_OFFSET);
  end
  ...
  always_comb begin
    wr_err = (reg_we &
              ((addr_hit[0] & (|(UART_MASTER_PERMIT[0] & ~reg_be))) |
               (addr_hit[1] & (|(UART_MASTER_PERMIT[1] & ~reg_be))) |
               (addr_hit[2] & (|(UART_MASTER_PERMIT[2] & ~reg_be))) |
               (addr_hit[3] & (|(UART_MASTER_PERMIT[3] & ~reg_be))) |
               (addr_hit[4] & (|(UART_MASTER_PERMIT[4] & ~reg_be))) |
               (addr_hit[5] & (|(UART_MASTER_PERMIT[5] & ~reg_be)))));
  end
