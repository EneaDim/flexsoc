module uart_host_bridge (
  ...
  logic parser_rst_ni;
  logic bus_rst_ni;
  logic response_rst_ni;

  // Split the host bridge reset by functional state island.  The three
  // branches are launched in parallel from the already synchronized host
  // reset, so asynchronous assertion is preserved and release latency stays
  // aligned across parser, request/bus, and response/TX state.
  prim_flop #(
    .Width      (1),
    .ResetValue (1'b0)
  ) u_parser_reset_stage (
    .clk_i (clk_i),
    .rst_ni(rst_ni),
    .d_i   (1'b1),
    .q_o   (parser_rst_ni)
  ...
  // ====== Parser state FFs ======
  always_ff @(posedge clk_i or negedge parser_rst_ni) begin
    if (!parser_rst_ni) begin
      rxf_st_q <= RXF_IDLE;
      idx_q    <= '0;
      sh_q     <= '0;
      op_q     <= '0;
  ...
  // ====== Host request/bus FFs ======
  always_ff @(posedge clk_i or negedge bus_rst_ni) begin
    if (!bus_rst_ni) begin
      addr_q  <= '0;
      wdata_q <= '0;
      be_q    <= 4'hF;
  ...
  // ====== Response/TX FFs ======
  always_ff @(posedge clk_i or negedge response_rst_ni) begin
    if (!response_rst_ni) begin
      rdata_q   <= '0;
      any_err_q <= 1'b0;
      tx_st_q   <= TX_IDLE;
      tx_idx_q  <= '0;
