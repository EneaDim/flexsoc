...
def render_core(hj: Hjson) -> str:
    ...
    lines: List[str] = []
    lines.append("// -----------------------------------------------------------------------------")
    lines.append("// THIS FILE IS AUTO-GENERATED. Edit the logic below the placeholders as needed.")
    lines.append("// -----------------------------------------------------------------------------")
    ...
    lines.append("")
    lines.append("  // TODO: add real IP I/Os")
    lines.append("  input  logic port_i,")
    lines.append("  output logic port_o")
    lines.append(");")
    lines.append("")
    lines.append("  //////////////")
    ...
    if signal_defs:
        ...
        lines.append("  // (no CSR-driven signals inferred yet)")
    lines.append("")
    lines.append("  //////////////")
    lines.append("  // CTRL2REG //")
    lines.append("  //////////////")
    lines.extend(ctrl2reg_assign if ctrl2reg_assign else ["  // (no assignments)"])
    lines.append("")
    lines.append("  //////////////")
    lines.append("  // REG2CTRL //")
    lines.append("  //////////////")
    lines.extend(reg2ctrl_assign if reg2ctrl_assign else ["  // (no assignments)"])
    lines.append("")
    lines.append("  // ---------------------------------------------------------------------------")
    lines.append("  // Example placeholder logic: 2-flop synchronizer")
    lines.append("  // Replace with your real core functionality.")
    lines.append("  // ---------------------------------------------------------------------------")
    lines.append("  prim_ff_2sync #(")
    lines.append("    .Width(1),")
    lines.append("    .ResetValue('0)")
    lines.append("  ) u_sync_name (")
    lines.append("    .clk_i (clk_i),")
    lines.append("    .rst_ni(rst_ni),")
    lines.append("    .d_i   (port_i),")
    lines.append("    .q_o   (port_o)")
    lines.append("  );")
    lines.append("")
...
def render_wrapper(hj: Hjson, itf: str) -> str:
    ...
    lines: List[str] = []
    lines.append("// -----------------------------------------------------------------------------")
    lines.append("// THIS FILE IS AUTO-GENERATED. It wires the register block with the core.")
    lines.append("// -----------------------------------------------------------------------------")
    lines.append(f"module {module}")
    lines.append(f"  import {reg_pkg}::*;")
    lines.append("(")
    lines.append("  // Clocks & Reset")
    lines.append("  input  clk_i,")
    lines.append("  input  rst_ni,")
    lines.append("")
    ...
    lines.append("")
    lines.append("  // Register block")
    lines.append(f"  {module}_reg_top u_{module}_reg (")
    lines.append("    .clk_i(clk_i),")
    lines.append("    .rst_ni(rst_ni),")
    lines.append(f"    {bus_conn_in}")
    lines.append(f"    {bus_conn_out}")
    lines.append("    .reg2hw(reg2hw),")
    lines.append("    .hw2reg(hw2reg),")
    lines.append("    .devmode_i(1'b1)")
    lines.append("  );")
    lines.append("")
    lines.append("  // Core")
    ...
    lines.append("    .reg2hw(reg2hw),")
    lines.append("    .hw2reg(hw2reg),")
    lines.append("    .port_i(port_i),")
    lines.append("    .port_o(port_o)")
    lines.append("  );")
    lines.append("")
    lines.append("endmodule")
    return "\n".join(lines)
