...
class CdcFlow:
    ...
    def _attribute_bool(item: Mapping[str, Any], name: str) -> bool:
        value = (CdcFlow._attribute_text(item, name) or "").lower()
        return value in {"1", "true", "yes", "on"} or (set(value) <= {"0", "1"} and value.endswith("1"))

    @staticmethod
    def _cdc_contract(item: Mapping[str, Any]) -> str | None:
        return (CdcFlow._attribute_text(item, _CDC_CONTRACT_ATTR) or "").lower() or None
    ...
    def _domain_bit_map(module: Mapping[str, Any], clocks: ClockConfig) -> dict[NetBit, str]:
        ...
        pending = [
            ...
            if CdcFlow._cdc_contract(cell) == "clock_gate"
        ]
        while pending:
            unresolved: list[tuple[str, Mapping[str, Any]]] = []
            progress = False
            for name, cell in pending:
                domain = CdcFlow._attribute_text(cell, _CDC_DOMAIN_ATTR)
                in_port = CdcFlow._attribute_text(cell, _CDC_CLOCK_IN_PORT_ATTR)
                out_port = CdcFlow._attribute_text(cell, _CDC_CLOCK_OUT_PORT_ATTR)
    ...
    def load_yosys_json(data: Mapping[str, Any], top: str, clocks: ClockConfig) -> DesignIR:
        ...
        for name, cell in module.get("cells", {}).items():
            ...
            clock_bits = CdcFlow._bits(cell, clk_port)
            data_bits = CdcFlow._bits(cell, data_port)
            q_bits = CdcFlow._bits(cell, "Q")
            if len(clock_bits) != 1 or not data_bits or not q_bits:
                continue
            async_bits = CdcFlow._bits(cell, async_port)
            sync_bits = CdcFlow._bits(cell, sync_port)
            reset_bit = async_bits[0] if async_bits else (sync_bits[0] if sync_bits else None)
            polarity = None
            if async_bits:
                polarity = CdcFlow._parameter_bool(cell, "ARST_POLARITY")
            elif sync_bits:
                polarity = CdcFlow._parameter_bool(cell, "SRST_POLARITY")
            sequential.append(
                SequentialElement(
                    name=name,
                    cell_type=cell_type,
        ...
        return DesignIR(
            ...
            ports=CdcFlow._port_intent(module, clocks),
    ...
    def _graph(ir: DesignIR) -> tuple[
        ...
        for name, cell in cells.items():
            if name in seq_by_name or CdcFlow._cdc_contract(cell) == "async_fifo":
                continue
        ...
        try:
            for port_name, direction in cell.get("port_directions", {}).items():
                ...
                for input_bit in cell.get("connections", {}).get(port_name, ()):
                    for endpoint, path in CdcFlow._upstream_sources(
                        input_bit,
                        sources=sources,
                        drivers=drivers,
                        cells=cells,
    ...
    def _sequential_dependencies(ir: DesignIR, clocks: ClockConfig) -> tuple[Crossing, ...]:
        ...
        for dest in ir.sequential:
            for index, bit in enumerate(dest.data_bits):
                ...
                for source, path in CdcFlow._upstream_sources(
                    bit,
                    sources=sources,
                    drivers=drivers,
                    cells=cells,
                    ...
                    crossing = Crossing(
                        source,
                        destination,
                        CdcFlow._relationship(clocks, source.clock_domain, dest.clock_domain),
                        path,
                    )
        ...
        return tuple(
            sorted(
                result.values(),
                key=lambda item: (
                    CdcFlow._endpoint_key(item.source),
                    CdcFlow._endpoint_key(item.destination),
                    item.path,
                ),
            )
        )
    ...
    def _reset_tree_stage_names(
        ...
        while changed:
            changed = False
            for seq in ir.sequential:
                if seq.name in stages or CdcFlow._reset_bit(seq) is None:
                    continue
                if len(seq.data_bits) != 1 or len(seq.q_bits) != 1:
                    continue
                expected = CdcFlow._reset_stage_deasserted_value(seq)
                if expected is None:
                    continue
                ...
                if parent is None or parent.name not in stages:
                    continue
                if CdcFlow._reset_bit(parent) != CdcFlow._reset_bit(seq):
                    continue
    ...
    def _reset_family_map(
        ...
        def family(seq: SequentialElement) -> str:
            ...
            active.add(seq.name)
            try:
                bit = CdcFlow._reset_bit(seq)
                if bit is None:
                    result = fallback
                elif bit in declared:
                    result = declared[bit]
                else:
                    upstream = CdcFlow._upstream_sources(
        ...
        return {
            seq.name: family(seq)
            for seq in ir.sequential
            if CdcFlow._reset_bit(seq) is not None
        }
    ...
    def analyze_domains(ir: DesignIR, clocks: ClockConfig) -> DomainAnalysis:
        ...
        reset_families = CdcFlow._reset_family_map(ir, dependencies)
    ...
    def _crossing_groups(crossings: tuple[Crossing, ...]) -> tuple[tuple[Crossing, ...], ...]:
        ...
        for crossing in crossings:
            groups.setdefault(CdcFlow._crossing_group_key(crossing), []).append(crossing)
        return tuple(
    ...
    def _control_source_endpoints(ir: DesignIR, seq: SequentialElement) -> tuple[Endpoint, ...]:
        ...
        for bit in enable_bits:
            for endpoint, _ in CdcFlow._upstream_sources(
                bit,
                sources=sources,
                drivers=drivers,
                cells=cells,
                ...
                if endpoint.kind == "seq":
                    result.add(endpoint)
        return tuple(sorted(result, key=CdcFlow._endpoint_key))
    ...
    def _input_paths(ir: DesignIR, bit: NetBit) -> tuple[tuple[Endpoint, tuple[str, ...]], ...]:
        ...
        return tuple(
            item
            for item in CdcFlow._upstream_sources(
                bit,
                sources=sources,
                drivers=drivers,
                cells=cells,
    ...
    def _check_synchronized_reconvergence(
        ...
        for (destination, origin), items in sorted(reconv.items(), key=lambda item: (CdcFlow._endpoint_key(item[0][0]), str(item[0][1]))):
    ...
    def _cdc_contract_findings(ir: DesignIR) -> tuple[list[DomainFinding], list[DomainFinding]]:
        ...
        domain_by_bit = CdcFlow._domain_bit_map(ir.module, clocks)
        known_domains = {domain.name for domain in ir.clocks}
        names = CdcFlow._net_names(ir.module)
        for name, cell in ir.module.get("cells", {}).items():
            contract = CdcFlow._cdc_contract(cell)
            ...
            if contract != "async_fifo":
                ...
                continue
            src_domain = CdcFlow._attribute_text(cell, _CDC_SOURCE_DOMAIN_ATTR)
            dst_domain = CdcFlow._attribute_text(cell, _CDC_DESTINATION_DOMAIN_ATTR)
            src_clk_port = CdcFlow._attribute_text(cell, _CDC_SOURCE_CLOCK_PORT_ATTR)
            dst_clk_port = CdcFlow._attribute_text(cell, _CDC_DESTINATION_CLOCK_PORT_ATTR)
            src_rst_port = CdcFlow._attribute_text(cell, _CDC_SOURCE_RESET_PORT_ATTR)
            dst_rst_port = CdcFlow._attribute_text(cell, _CDC_DESTINATION_RESET_PORT_ATTR)
            partial_reset_safe = CdcFlow._attribute_bool(cell, _CDC_PARTIAL_RESET_SAFE_ATTR)
    ...
    def _finding_record(finding: DomainFinding, index: int) -> dict[str, Any]:
        return {
            "id": CdcFlow._finding_code(finding, index),
            "scope": finding.scope,
            "status": finding.status,
            "classification": finding.classification,
            "primary": finding.primary,
            "crossings": [CdcFlow._crossing_dict(item) for item in finding.crossings],
    ...
    def _finding_records(findings: Sequence[DomainFinding]) -> list[dict[str, Any]]:
        return [CdcFlow._finding_record(finding, index) for index, finding in enumerate(findings, 1)]
    ...
    def _primary_by_crossing(findings: Sequence[DomainFinding]) -> dict[tuple[str, int, str, int], dict[str, Any]]:
        ...
        for index, finding in enumerate(findings, 1):
            if not finding.primary:
                continue
            record = CdcFlow._finding_record(finding, index)
            for crossing in finding.crossings:
                result[CdcFlow._crossing_key(crossing)] = record
        return result
    ...
    def _summary(ir: DesignIR, analysis: DomainAnalysis, result: ComprehensiveAnalysis) -> dict[str, Any]:
        ...
        return {
            ...
            "cdc": {
                ...
                **cdc_counts,
                "classifications": CdcFlow._class_counts(result.cdc),
                "pairs": CdcFlow._pair_counts(analysis.clock_crossings),
            },
            "rdc": {
                "raw_crossings": len(analysis.reset_crossings),
                **rdc_counts,
                "classifications": CdcFlow._class_counts(result.rdc),
                "pairs": CdcFlow._pair_counts(analysis.reset_crossings, reset=True),
            },
            "setup": {**setup_counts, "classifications": CdcFlow._class_counts(result.setup)},
            "glitch": {**glitch_counts, "classifications": CdcFlow._class_counts(result.glitch)},
    ...
    def _crossing_records(
        ...
        primary = CdcFlow._primary_by_crossing(findings)
        records = []
        for crossing in crossings:
            record = CdcFlow._crossing_dict(crossing)
            finding = primary.get(CdcFlow._crossing_key(crossing))
            if finding is None:
    ...
    def write_reports(
        ...
        *,
        ...
        cdc_records = CdcFlow._finding_records(classified.cdc)
        rdc_records = CdcFlow._finding_records(classified.rdc)
        setup_records = CdcFlow._finding_records(classified.setup)
        glitch_records = CdcFlow._finding_records(classified.glitch)
        summary = CdcFlow._summary(ir, analysis, classified)
        obligations = [
            {
            ...
            **summary,
            ...
            "cdc": {
                **summary["cdc"],
                ...
                "crossings": CdcFlow._crossing_records(analysis.clock_crossings, classified.cdc),
            },
            "rdc": {
                **summary["rdc"],
                "findings": rdc_records,
                "crossings": CdcFlow._crossing_records(analysis.reset_crossings, classified.rdc),
            },
        ...
        lines = [
            ...
            "",
            "[CDC]",
            *(CdcFlow._finding_line(record) for record in cdc_records),
            "",
            "[RDC]",
            *(CdcFlow._finding_line(record) for record in rdc_records),
            "",
            "[setup]",
            *(CdcFlow._finding_line(record) for record in setup_records),
            "",
            "[glitch]",
    ...
    def collect_cdc_debug(
        *,
        ...
        guard_state = CdcFlow._contract_guard_state(
            source_contracts=source_contracts,
            design_contracts=design_contracts,
            guard_present=guard_present,
        )
        scopes = {name: CdcFlow._scope_payload(summary, name) for name in ("setup", "glitch", "cdc", "rdc")}
        obligations = [
            {
        ...
        triage = CdcFlow._triage(scopes, diagnoses)
        return {
