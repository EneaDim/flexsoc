"""FlexSoC structural CDC/RDC extraction, analysis, and reporting."""
...
from flexsoc.backend.output import print_label, print_log, print_script, print_status_label
_T = TypeVar("_T")
...
def _classify_synchronizer_crossing(
    ...
    # Two clean destination-domain stages are sufficient for structural
    # recognition. Do not absorb downstream functional registers into the
    # synchronizer merely because they form an exclusive FF-to-FF chain.
...
def classify_synchronizers(
    ...
    Vectored transfers are intentionally left to the multibit/Gray/control
    classifiers; treating each bus bit as an independent synchronizer creates
    false safety and false errors.
    """
...
def classify_cdc_rdc(ir: DesignIR, analysis: DomainAnalysis) -> ComprehensiveAnalysis:
    ...
    The classifier separates structural safety from protocol properties that
    require assertions. REVIEW findings therefore carry explicit obligations
...
def _clock_values(args: argparse.Namespace) -> ClockConfig:
    return clock_config(
        {
...
def run_analysis(args: argparse.Namespace) -> int:
    ...
    def extract() -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            [args.yosys, "-ql", str(extract_log), str(script)],
            check=False,
            text=True,
        )
...
def setup_analysis(args: argparse.Namespace) -> int:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    setup = sub.add_parser("setup", help="Generate the pre-technology Yosys extraction script.")
    _common_run_args(setup)
    setup.add_argument("--repo-root", required=True)
    setup.add_argument("--filelist", action="append", default=[], required=True)

    run = sub.add_parser("run", help="Run structural CDC/RDC, setup, glitch, and protocol-candidate checks.")
    _common_run_args(run)
    run.add_argument("--analysis-dir", required=True)
    run.add_argument("--log-dir", required=True)
    run.add_argument("--yosys", default="yosys")
    run.add_argument("--n-clocks", default="1")
    run.add_argument("--clock-domains", default="")
    run.add_argument("--clock-relationships", default="")
    run.add_argument("--clk-period", default="10")
    run.add_argument("--heartbeat", type=float, default=5.0)
    run.add_argument("--strict", action="store_true", help="Return non-zero when structural ERROR findings exist.")
