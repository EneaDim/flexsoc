...
from .sta import (
    ACTIVITY_PERCENT_RE, ACTIVITY_RE, FLOAT_RE, POWER_RE, SDF_MODES, SignoffContext, StaAnalysis,
)
...
class ActivitySpec:
    ...
    @property
    def stem(self) -> str:
        return f"{self.top}_{self.pdk}_{self.test}_{self.backend}_{StaAnalysis.scenario_corner(self.mode)}"

    @property
    def workload(self) -> str:
        """Return the sign-off workload name using the aligned PVT corner."""

        return f"{self.test}_{self.backend}_{StaAnalysis.scenario_corner(self.mode)}"
...
class PowerAnalysis:
    ...
    def _qualified_spec(
        ...
        *,
        ...
        report = PowerAnalysis._post_syn_report(project_root, values, test=test, backend=backend, mode=mode)
        ...
        expected_corner = StaAnalysis.scenario_corner(mode)
        ...
        if action == "single":
            return (
                PowerAnalysis._qualified_spec(
                    root,
                    values,
                    test=PowerAnalysis._one(tests, "POWER_TEST_NAME"),
                    backend=PowerAnalysis._one(backends, "POWER_GLS_BACKEND"),
                    mode=PowerAnalysis._one(modes, "POWER_TIMING_MODE"),
                ),
            )

        available = PowerAnalysis._available_gls(root, values)
        if not available:
        ...
        for test, mode in sorted(grouped, key=lambda item: (mode_order[item[1]], item[0])):
            ...
            for backend in backend_order:
                if backend not in grouped[(test, mode)]:
                    continue
                try:
                    spec = PowerAnalysis._qualified_spec(
        ...
        try:
            with source.open("r", encoding="utf-8", errors="replace") as src, temporary.open(
                ...
                for line in src:
                    ...
                    identifier = PowerAnalysis._vcd_value_identifier(stripped)
            ...
            if not PowerAnalysis._valid_vcd(temporary):
    ...
    def _resolve_vcd_scope(
        ...
        scopes = PowerAnalysis._vcd_scopes(vcd)
        if not scopes:
    ...
    @staticmethod
    def _convert_fst(
        *, executable: Path, source: Path, output: Path, log: Path, runner=None,
        on: str = "local",
    ) -> str:
        """Convert one FST through ToolRunner, preserving the stdout fallback."""

        from flexsoc.backend.core import CommandRequest, ToolRunner

        active_runner = runner or ToolRunner(project_root=source.parent)
        output.unlink(missing_ok=True)
        attempts: list[tuple[str, int, int]] = []

        request = CommandRequest(
            (str(executable), "-f", str(source), "-o", str(output)),
            source.parent, {}, log, inputs=(source,), outputs=(output,),
        )
        result = active_runner.run(request, on=on)
        size = output.stat().st_size if output.is_file() else 0
        attempts.append(("named-output", result.returncode, size))
        if result.returncode == 0 and PowerAnalysis._valid_vcd(output):
            log.unlink(missing_ok=True)
            return "named-output"

        first_log = log.read_text(encoding="utf-8", errors="replace") if log.is_file() else ""
        output.unlink(missing_ok=True)
        retry_log = log.with_name(log.stem + "_stdout.log")
        request = CommandRequest(
            (str(executable), str(source)), source.parent, {}, retry_log,
            inputs=(source,), outputs=(output,), stdout=output,
        )
        result = active_runner.run(request, on=on)
        retry_text = retry_log.read_text(encoding="utf-8", errors="replace") if retry_log.is_file() else ""
        log.write_text(first_log + "\n[fst2vcd] retry=stdout\n" + retry_text, encoding="utf-8")
        retry_log.unlink(missing_ok=True)
        size = output.stat().st_size if output.is_file() else 0
        attempts.append(("stdout", result.returncode, size))
        if result.returncode == 0 and PowerAnalysis._valid_vcd(output):
            log.unlink(missing_ok=True)
            return "stdout"
    ...
    def _activity_vcd(
        ...
        if spec.wave.suffix.lower() == ".vcd":
            if not PowerAnalysis._valid_vcd(spec.wave):
        ...
        executable = values.get("FST2VCD", "fst2vcd")
        capture_dir.mkdir(parents=True, exist_ok=True)
        output = capture_dir / f"{spec.stem}.vcd"
        log = capture_dir / f"{spec.stem}_fst2vcd.log"
        method = PowerAnalysis._convert_fst(
            executable=Path(executable), source=spec.wave, output=output, log=log,
            runner=runner, on=on,
        )
        output, events, changes, remapped = PowerAnalysis._sanitize_vcd_for_opensta(output, output)
        if events:
            method += f"+opensta-event-filter(events={events},changes={changes})"
        if remapped:
            method += f"+opensta-id-remap(ids={remapped})"
    ...
    def _power_instance_rows(path: Path) -> list[dict[str, Any]]:
        ...
        if text.lstrip().startswith(("[", "{")):
            ...
            for line in text.splitlines():
                ...
                name = PowerAnalysis._instance_name(prefix)
        ...
        for item in items:
            ...
            internal = PowerAnalysis._item_number(item, "internal", "internal_power")
            switching = PowerAnalysis._item_number(item, "switching", "switching_power")
            leakage = PowerAnalysis._item_number(item, "leakage", "leakage_power")
            total = PowerAnalysis._item_number(item, "total", "total_power") or internal + switching + leakage
            rows.append(
                {
                    "instance": name,
                    "internal": internal,
        ...
        if not rows:
            raise ValueError(
                ...
                f"excerpt={PowerAnalysis._power_report_excerpt(text)}"
            )
        return rows
    ...
    def _annotate_power_summary(report_dir: Path, name: str = "power.rpt") -> None:
        ...
        values = PowerAnalysis._power_values(text)
        lines = [
            "",
    ...
    @staticmethod
    def _item_number(item: Mapping[str, Any], *keys: str) -> float:
        for key in keys:
            value = item.get(key)
    ...
    def _write_activity_table(
        ...
        for key, item in corners.items():
            ...
            if raw_report:
                ...
                report_name = "n/a"
            values = [PowerAnalysis._metric(item, name) for name in ("internal_w", "switching_w", "leakage_w", "total_w")]
            ...
            if analysis == "fusion_analysis":
                lines.append(
                    ...
                    f"{PowerAnalysis._metric(item, 'wns'):>9} {PowerAnalysis._metric(item, 'tns'):>9} "
    ...
    def analyze_activity_spec(
        ...
        *,
        ...
        capture, conversion_log, conversion_method = (
            PowerAnalysis._activity_vcd(*activity_args)
            if runner is None
            else PowerAnalysis._activity_vcd(*activity_args, runner=runner, on=on)
        )
        requested_scope = values.get("POWER_VCD_SCOPE", "auto").strip() or "auto"
        scope, scopes = PowerAnalysis._resolve_vcd_scope(
            capture,
        ...
        configured_corners = StaAnalysis._selection(
            values.get("SIGNOFF_CORNERS"), "ss tt ff", tuple(liberties), "sign-off corner"
        )
        aligned_corner = StaAnalysis.scenario_corner(spec.mode)
        ...
        modes = (
            StaAnalysis._selection(values.get("STA_MODES"), "setup hold", ("setup", "hold"), "STA mode")
            if analysis == "fusion_analysis"
            else ("",)
        )
        ...
        for corner in selected_corners:
            ...
            for mode in modes:
                ...
                try:
                    ...
                    rc = StaAnalysis._execute_script(project_root, values, **execute_kwargs)
                    ...
                    numbers = PowerAnalysis._power_values(report_text) or PowerAnalysis._power_values(text)
                    annotated = PowerAnalysis._activity_count(report_text)
                    if annotated is None:
                        annotated = PowerAnalysis._activity_count(text)
                    annotation_percent = PowerAnalysis._activity_percent(report_text)
                    has_activity = (
                    ...
                    corners[key] = {
                        ...
                        **(StaAnalysis._timing_values(report_text) if analysis == "fusion_analysis" else {}),
                        **fusion_details,
                        **numbers,
                    }
                    if ok:
                        ...
                        reason = (
                            f"OpenSTA {StaAnalysis._returncode_text(rc)}"
                            if rc
        ...
        table = PowerAnalysis._write_activity_table(analysis, workload_root, spec.workload, corners)
        return {
            "schema_version": 4,
            "analysis": analysis,
    ...
    def execute_activity(
        ...
        *,
        ...
        specs = PowerAnalysis.discover_specs(action, values, project_root)
        ...
        for index, spec in enumerate(specs, 1):
            ...
            try:
                ...
                report = PowerAnalysis.analyze_activity_spec(analysis, project_root, values, spec, **kwargs)
            except (OSError, ValueError) as exc:
        ...
        summary = {
            ...
            "total": len(reports),
            "reports": [PowerAnalysis._activity_qor(report) for report in reports],
        }
