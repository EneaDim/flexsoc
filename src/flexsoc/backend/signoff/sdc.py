...
class Sdc:
    """Own SDC scaffold, parsing, and clock/I/O timing semantics."""
    ...
    def infer_io_groups(top_file: Path, clocks: ClockConfig) -> tuple[tuple[SdcIoGroup, ...], tuple[str, ...]]:
        ...
        for port in RtlFlow.parse_ports(path):
    ...
    def render_sdc_scaffold(
        ...
        *,
        ...
        return templates.render(
            ...
            top=top,
            clock_lines=Sdc._clock_lines(clocks),
            clock_quality_lines=Sdc._clock_quality_lines(clocks),
            relationship_lines=Sdc._relationship_lines(clocks),
            input_timing_lines=Sdc._io_timing_lines(
                clocks, io_groups, unassigned_ports, io_delay_pct, direction="input"
            ),
            output_timing_lines=Sdc._io_timing_lines(
                clocks, io_groups, unassigned_ports, io_delay_pct, direction="output"
            ),
            functional_mode_lines=Sdc._functional_mode_lines(clocks),
    ...
    def init_sdc(
        ...
        *,
        ...
        if clocks.multiclock and top_file is not None:
            groups, unassigned = Sdc.infer_io_groups(top_file, clocks)
        return Sdc.write_sdc(
            path,
            Sdc.render_sdc_scaffold(
                top,
                clocks,
    ...
    def read_clock_config(path: Path, reset_config: ClockConfig) -> ClockConfig:
        ...
        commands = Sdc._commands(path.read_text(encoding="utf-8", errors="replace"))
        ...
        for command in commands:
            ...
            period = Sdc._float_option(command, "-period")
            signal = Sdc._port_name(command)
        ...
        for command in generated_commands:
            name_match = re.search(r"(?:^|\s)-name\s+([^\s]+)", command)
            divide = Sdc._float_option(command, "-divide_by")
        ...
        for command in commands:
            name = Sdc._clock_name(command)
            if name not in by_name:
                continue
            domain = by_name[name]
            if command.startswith("set_clock_latency "):
                value = Sdc._float_option(command, "-source")
                if value is None:
    ...
    def read_io_environment(path: Path) -> SdcIoEnvironment:
        ...
        driving_cell: str | None = None
        load: float | None = None
        for command in Sdc._commands(Path(path).read_text(encoding="utf-8", errors="replace")):
