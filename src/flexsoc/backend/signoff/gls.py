...
class GateLevelSimulation:
    ...
    def compile(self, *, test: str, timing: str = "zero", backend: str = "sv", on: str = "local") -> int:
        ...
        return GateLevelSimulation.execute("compile", self.stage, self.project_root, values, runner=self.runner, on=on)

    def run(self, *, test: str, timing: str = "zero", backend: str = "sv", on: str = "local") -> int:
        """Run one GLS workload."""

        values = {**self.values, "TEST_NAME": test, "TIMING_MODE": timing, "GLS_BACKEND": backend}
        return GateLevelSimulation.execute("sim", self.stage, self.project_root, values, runner=self.runner, on=on)

    def run_all(self, *, on: str = "local") -> int:
        """Run every configured test/timing scenario."""

        return GateLevelSimulation.execute_all(self.project_root, self.values, stage=self.stage, runner=self.runner, on=on)

    @staticmethod
    def timing_scenario(mode: str) -> str:
        """Return the user-facing GLS scenario name for one timing mode."""

        return StaAnalysis.scenario_corner(mode) if mode in SDF_MODES else mode
        ...
        for line in text.splitlines():
            ...
            if match:
                ...
                for name in dict.fromkeys(identifiers):
                    base = GateLevelSimulation._delayed_input(name)
                    if base:
    ...
    def _simulation_models(values: Mapping[str, str], paths: GateSimPaths, timing: TimingConfig) -> tuple[Path, ...]:
        ...
        models = tuple(path for path in GateLevelSimulation._split_paths(values.get("PRIM")) if path.is_file())
        pdk = values.get("PDK", "").strip().lower()
        path_only = timing.uses_sdf and GateLevelSimulation._truth(values.get("SDF_ICARUS_PATH_ONLY", "1"))
        ihp = pdk == "ihp-sg13g2"
        ...
        for source in models:
            ...
            if path_only and not (needs_delayed_binding or needs_timing_cleanup or needs_ihp_cleanup):
                resolved.append(source)
                continue
            rendered, stats = GateLevelSimulation._icarus_path_delay_model(
                text,
                remove_ifnone=ihp,
    ...
    def _discover_pnr_netlist(pnr_dir: Path, top: str, platform: str) -> Path | None:
        return ImplementationFlow.resolve_orfs_artifact(pnr_dir, "results", top, "6_final.v", platform)

    @staticmethod
    def _post_syn_sdf(layout, top: str, values: Mapping[str, str], mode: str) -> Path:
        default_corner = StaAnalysis.scenario_corner(mode)
    ...
    def resolve_paths(project_root: Path, values: Mapping[str, str], stage: str) -> GateSimPaths:
        ...
        driver = GateLevelSimulation._driver(values)
        timing = GateLevelSimulation.timing_config(values)
        ...
        tb = (
            ...
            if driver == "sv"
            else GateLevelSimulation._cocotb_wrapper(run_root, top)
        )
        stage_dir = layout.post_syn_sim_dir if stage == "post_syn" else layout.post_impl_sim_dir
        ...
        if stage == "post_syn":
            ...
            netlist = explicit_netlist or GateLevelSimulation._discover_pnr_netlist(layout.pnr_dir, top, platform)
            if netlist is None:
                raise ValueError(
                    "post-implementation netlist not found; run pnr first or pass --set NETLIST=/path/to/6_final.v"
                )
        ...
        if timing.uses_sdf:
            if explicit_sdf:
                ...
                sdf = GateLevelSimulation._post_syn_sdf(layout, top, values, timing.mode)
            else:
        ...
        tag = f"{test}_{driver}_{GateLevelSimulation.timing_scenario(timing.mode)}"
        wave = (
        ...
        log_dir = layout.post_syn_log_dir if stage == "post_syn" else layout.post_impl_log_dir
    ...
    def _format_time(seconds: Decimal) -> str:
        for unit in ("s", "ms", "us", "ns", "ps", "fs"):
            ...
            if value == value.to_integral_value():
                return f"{GateLevelSimulation._format_decimal(value)}{unit}"
        return f"{GateLevelSimulation._format_decimal(seconds / _TIME_FACTORS['fs'])}fs"
    ...
    def _unit_delay_resolution(
        ...
        requested_literal = GateLevelSimulation._unit_delay_literal(values)
        requested = GateLevelSimulation._time_seconds(requested_literal)
        ...
        for path in models:
            ...
            for match in _TIMESCALE.finditer(text):
                timeunits.add(GateLevelSimulation._time_seconds(match.group("unit")))
                precisions.append(GateLevelSimulation._time_seconds(match.group("precision")))
        if not timeunits:
            timeunits = {_TIME_FACTORS["ns"]}
            precisions = [_TIME_FACTORS["ps"]]
        if len(timeunits) != 1:
            rendered = ", ".join(sorted(GateLevelSimulation._format_time(value) for value in timeunits))
            raise ValueError(
        ...
        return UnitDelayResolution(
            ...
            effective=GateLevelSimulation._format_time(effective),
            define=GateLevelSimulation._format_decimal(scalar),
            model_timeunit=GateLevelSimulation._format_time(timeunit),
            model_precision=GateLevelSimulation._format_time(precision),
        )
    ...
    def _compile_timing_args(
        ...
        if timing.mode == "unit":
            delay = GateLevelSimulation._unit_delay_resolution(values, models)
            return [
                "-DFUNCTIONAL",
                "-gno-specify",
    ...
    def _icarus_register_package(source: Path, interface: str) -> str:
        ...
        types = "\n\n".join(GateLevelSimulation._packed_type(text, name) for name in names)
        return (
    ...
    def _register_packages(values: Mapping[str, str], paths: GateSimPaths) -> tuple[Path, ...]:
        ...
        interface = RegsFlow.normalize_register_interface(values.get("REG_ITF", "tlul"))
        if interface == "tlul":
            return ()
        ...
        for source in sources:
            target = target_dir / source.name
            target.write_text(GateLevelSimulation._icarus_register_package(source, interface), encoding="utf-8")
    ...
    def compile_command(project_root: Path, values: Mapping[str, str], stage: str, paths: GateSimPaths) -> list[str]:
        ...
        if GateLevelSimulation._driver(values) != "sv":
        ...
        models = GateLevelSimulation._simulation_models(values, paths, timing)
        ...
        command = [
            ...
            *GateLevelSimulation._compile_timing_args(timing, values, models),
            *(["-ginterconnect"] if stage == "post_impl" and timing.uses_sdf else []),
            "-s",
        ...
        command += [str(path) for path in GateLevelSimulation._register_packages(values, paths)]
    ...
    def resolve_test_inputs(values: Mapping[str, str], paths: GateSimPaths) -> GateSimInputs:
        ...
        return GateSimInputs(
            test_name,
            test_root.resolve(),
            GateLevelSimulation._selected_input(values, test_dir, "REGCFG", "config.regs"),
            GateLevelSimulation._selected_input(values, test_dir, "DATA_IN", "data_in.vec"),
            GateLevelSimulation._selected_input(values, test_dir, "DATA_OUT", "data_out.vec"),
        )

    @staticmethod
    def _selected_input(values: Mapping[str, str], test_dir: Path, key: str, filename: str) -> Path:
        return (
    ...
    def run_command(values: Mapping[str, str], paths: GateSimPaths) -> list[str]:
        ...
        inputs = GateLevelSimulation.resolve_test_inputs(values, paths)
        command += [
    ...
    def cocotb_command(
        ...
        inputs = GateLevelSimulation.resolve_test_inputs(values, paths)
        model_paths = GateLevelSimulation._simulation_models(values, paths, timing)
        models = " ".join(str(path) for path in model_paths)
        packages = " ".join(str(path) for path in GateLevelSimulation._register_packages(values, paths))
        unit_delay = GateLevelSimulation._unit_delay_resolution(values, model_paths) if timing.unit_delay else None
        ...
        command = [
            ...
            f"GLS_INTERCONNECT={int(stage == 'post_impl' and timing.uses_sdf)}",
    ...
    def _run(
        ...
        *,
        ...
        """Run one GLS command through ToolRunner."""

        from flexsoc.backend.core.runtime.execution import CommandRequest, ToolRunner
    ...
    def _write_report(
        ...
        *,
        ...
        annotation = GateLevelSimulation.sdf_annotation_summary(text) if timing.uses_sdf else None
        strict = GateLevelSimulation._truth(values.get("SDF_STRICT", "1"))
        final_rc = rc
        ...
        unit_delay = (
            GateLevelSimulation._unit_delay_resolution(values, GateLevelSimulation._split_paths(values.get("PRIM")))
        ...
        report: dict[str, object] = {
            ...
            "timing_model": (
                ...
                if timing.uses_sdf and GateLevelSimulation._truth(values.get("SDF_ICARUS_PATH_ONLY", "1"))
    ...
    def execute_all(project_root: Path, values: Mapping[str, str], stage: str = "post_syn", *, runner=None, on: str = "local") -> int:
        ...
        layout = PDKRunLayout.from_values(project_root, values)
        tests = GateLevelSimulation._all_test_names(values, layout.run_root)
        ...
        backend = GateLevelSimulation._driver(values)
        modes = GateLevelSimulation._selector_values(
        ...
        for index, (test, mode) in enumerate(cases, 1):
            ...
            print(
                ...
                f"test={test} backend={backend} scenario={GateLevelSimulation.timing_scenario(mode)}"
                + (f" sdf_mode={mode}" if mode in SDF_MODES else ""),
                flush=True,
            )
            paths = GateLevelSimulation.resolve_paths(project_root, case_values, stage)
            try:
                rc = GateLevelSimulation.execute(
                    "sim", stage, project_root, case_values, runner=runner, on=on
                )
                    ...
                    json.dumps(
                        {
                            ...
                            "backend": backend,
                            "timing_mode": mode,
                            "scenario": GateLevelSimulation.timing_scenario(mode),
                            "reason": str(exc),
                        },
                        indent=2,
                        sort_keys=True,
        ...
        summary = {
            ...
            "scenarios": [GateLevelSimulation.timing_scenario(mode) for mode in modes],
            "passed": len(cases) - failures,
            "failed": failures,
            "total": len(cases),
            "reports": reports,
        }
        summary_dir = layout.post_syn_sim_dir if stage == "post_syn" else layout.post_impl_sim_dir
        summary_path = summary_dir / f"summary_{backend}.json"
        summary_path.parent.mkdir(parents=True, exist_ok=True)
        summary_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")

        scenario_names = [GateLevelSimulation.timing_scenario(mode) for mode in modes]
    ...
    def _command_files(
        ...
        timing = GateLevelSimulation.timing_config(values)
        tests = GateLevelSimulation.resolve_test_inputs(values, paths)
        models = GateLevelSimulation._simulation_models(values, paths, timing)
        if runtime:
    ...
    def execute(action: str, stage: str, project_root: Path, values: Mapping[str, str], *, runner=None, on: str = "local") -> int:
        ...
        driver = GateLevelSimulation._driver(values)
        if driver == "sv":
            compile_cmd = GateLevelSimulation.compile_command(project_root, values, stage, paths)
            compile_log = paths.log.with_name(paths.log.stem + "_compile.log")
            rc = GateLevelSimulation._run(
                compile_cmd, cwd=project_root, log=compile_log,
                inputs=GateLevelSimulation._command_files(project_root, values, paths, runtime=False),
                outputs=(paths.executable,), runner=runner, on=on,
            )
            if rc:
                paths.log.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(compile_log, paths.log)
                rc, _ = GateLevelSimulation._write_report(paths, values, stage, rc, phase="compile")
                return rc
            if action == "compile":
                return rc
            paths.wave.parent.mkdir(parents=True, exist_ok=True)
            rc = GateLevelSimulation._run(
                GateLevelSimulation.run_command(values, paths),
                cwd=project_root,
                log=paths.log,
                env=GateLevelSimulation.run_environment(values),
                inputs=GateLevelSimulation._command_files(project_root, values, paths, runtime=True),
                outputs=(paths.wave,), runner=runner, on=on,
            )
        else:
            command = GateLevelSimulation.cocotb_command(action, values, stage, paths)
            ...
            if action == "compile":
                return rc
            if rc == 0:
                GateLevelSimulation._normalize_cocotb_wave(paths, values)

        rc, report = GateLevelSimulation._write_report(paths, values, stage, rc)
        print(
