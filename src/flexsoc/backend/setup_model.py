...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Generate an editable model and vector tests.")
...
def main(argv: list[str] | None = None) -> int:
    """Write the model and, when requested, regenerate tests from it."""
    ...
    if args.tests_dir:
        generate_tests(model_path, Path(args.tests_dir))
    print(f"Model: {model_path.resolve()}")
    if args.tests_dir:
        print(f"Tests: {Path(args.tests_dir).resolve()}")
    return 0
