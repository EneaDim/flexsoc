...
def load_toolchain_lock(root: Path) -> dict[str, str]:
    ...
    path = root / "src" / "flexsoc" / "backend" / "core" / "toolchain.lock"
    if not path.is_file():
        return {}
    values: dict[str, str] = {}
...
class Toolchain:
    ...
    def doctor(self) -> dict[str, object]:
        return collect(self.project_root)

    def run_doctor(self, *, as_json: bool = False) -> int:
        return run(self.project_root, as_json=as_json)

    def versions(self) -> dict[str, str]:
        return load_toolchain_lock(self.project_root)
