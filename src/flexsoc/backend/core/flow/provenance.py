...
class Provenance:
    ...
    @classmethod
    def _path_sha256(cls, path: Path) -> str | None:
        ...
        for item in sorted(candidate for candidate in path.rglob("*") if candidate.is_file()):
            ...
            digest.update(bytes.fromhex(cls._file_sha256(item) or ""))
    ...
    @staticmethod
    def summary(states: Mapping[str, str]) -> dict[str, Any]:
    ...
    def _snapshot(self, paths: Sequence[Path]) -> list[dict[str, str | None]]:
        return [{"path": self._key(path), "sha256": self._path_sha256(path)} for path in paths]
    ...
    def _fingerprint(record: Mapping[str, Any]) -> str:
        ...
        return Provenance._json_sha256(payload)
    ...
    def current_fingerprint(
        ...
        current = {
            ...
            "generated": [
                {
                    "path": item.get("path"),
                    "effective_sha256": self._path_sha256(
    ...
    def state(
        ...
        for item in generated:
            current = self._path_sha256(self._resolve(str(item.get("path", ""))))
            if current is None:
                return "INVALID"
    ...
    def validate(
        ...
        for item in record["generated"]:
            item["effective_sha256"] = self._path_sha256(self._resolve(item["path"]))
        record["fingerprint"] = self._fingerprint(record)
        self._write(data)
