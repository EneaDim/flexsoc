...
from ..flow.target import BACKEND_TARGETS
...
_DIRECT_STAGE_KEYS = {
    ...
    "gls_post_impl": "sim_post_impl_all",
    "sta_post_impl": "sta_post_impl",
    "power_post_impl": "power_analysis_post_impl_all",
    "fusion_post_impl": "fusion_analysis_post_impl_all",
}
...
class ShowRenderer:
    ...
    def load_spec(run: Path, key: str) -> ShowDocument:
        ...
        path = ShowRenderer.spec_root(run) / filename
        if not path.is_file():
    ...
    def files(run: Path, *, top: str, pdk: str) -> tuple[dict[str, object], ...]:
        ...
        for path in sorted(item for item in run.rglob("*.json") if item.is_file()):
            if not ShowRenderer._important_json(run, path, top=top):
                continue
            relative = path.relative_to(run).as_posix()
            try:
                data = json.loads(path.read_text(encoding="utf-8"))
                kind = ShowRenderer.classify(path, data)
                error = None
            except (OSError, json.JSONDecodeError) as exc:
                kind = "invalid"
                error = str(exc)
    ...
    def load_file(run: Path, key: str) -> ShowDocument:
        ...
        relative = path.relative_to(run).as_posix()
        return ShowDocument(relative, relative, path, data, data, ShowRenderer.classify(path, data))
    ...
    def catalog(run: Path, *, top: str, pdk: str) -> dict[str, ShowSpec]:
        ...
        specs = (
            ...
            ShowSpec("physical_signoff", "Physical signoff", signoff / "post_impl" / "physical" / "summary.json"),
            ShowSpec("gls_post_impl", "Post-implementation GLS matrix", sim / "post_impl" / pdk / "summary_sv.json"),
            ShowSpec("sta_post_impl", "Post-implementation STA", signoff / "post_impl" / "sta" / "summary.json"),
            ShowSpec("power_estimate_post_impl", "Post-implementation vectorless power", signoff / "post_impl" / "power" / "estimate" / "summary.json"),
            ShowSpec("power_post_impl", "Post-implementation activity power", signoff / "post_impl" / "power" / "analysis" / "summary.json"),
            ShowSpec("fusion_post_impl", "Post-implementation timing/power fusion", signoff / "post_impl" / "fusion" / "summary.json"),
        )
    ...
    def _available(spec: ShowSpec) -> bool:
        ...
        try:
            ...
            ShowRenderer._select(data, spec.selector)
        ...
        return tuple(
            {
                ...
                "available": ShowRenderer._available(spec),
            }
            for spec in ShowRenderer.catalog(run, top=top, pdk=pdk).values()
        )
    ...
    def load(run: Path, *, top: str, pdk: str, key: str) -> ShowDocument:
        ...
        specs = ShowRenderer.catalog(run, top=top, pdk=pdk)
        ...
        try:
            data = ShowRenderer._select(root, spec.selector)
        except KeyError as exc:
            selector = ".".join(spec.selector)
            raise KeyError(f"{spec.path} has no JSON section {selector!r}") from exc
        return ShowDocument(spec.key, spec.title, spec.path, data, root, ShowRenderer.classify(spec.path, root))
    ...
    def issues(run: Path, *, top: str, pdk: str) -> tuple[dict[str, object], ...]:
        ...
        if qualification_path.is_file():
            ...
            if isinstance(evidence, Mapping):
                for stage, raw in evidence.items():
                    status = ShowRenderer._status(raw)
        ...
        for key, stage in _DIRECT_STAGE_KEYS.items():
            if stage in qualified_stages:
                continue
            try:
                document = ShowRenderer.load(run, top=top, pdk=pdk, key=key)
            except (FileNotFoundError, KeyError, ValueError):
                continue
            data = document.data
            status = ShowRenderer._status(data.get("status")) if isinstance(data, Mapping) else None
            if status not in {None, "PASS", "WAIVED"}:
    ...
    @staticmethod
    def load_and_render(run: Path, *, top: str, pdk: str, key: str, console: Console | None = None) -> ShowDocument:
    ...
    def __init__(self, console: Console, error_console: Console | None = None):
    ...
    def command(
        ...
        *,
        ...
        """Render one `fx show` request from canonical run artifacts."""

        raw_action = (args[0] if args else "keys").strip()
        action = raw_action.lower().replace("-", "_")
        if len(args) > 1:
            self.error_console.print(
                "[red]fx show accepts one key/path; use `fx show files` to list documents[/red]"
            )
            return 2
        if action in {"keys", "list"}:
            rows = ShowRenderer.keys(run, top=top, pdk=pdk)
            if as_json:
                print(json.dumps(rows, indent=2))
                return 0
            table = Table(title=f"FlexSoC reports · {run.parent.name}/{run.name} · {pdk}", header_style="bold white", expand=True)
            ...
            self.console.print(
                "[grey70]Use `fx show files` for every important JSON and `fx show all` to render them all.[/grey70]"
            )
            return 0
        if action in {"files", "documents"}:
            rows = ShowRenderer.files(run, top=top, pdk=pdk)
            if as_json:
                print(json.dumps(rows, indent=2))
                return 0
            table = Table(title=f"Important JSON · {run.parent.name}/{run.name}", header_style="bold white", expand=True)
            ...
            for item in rows:
                table.add_row(
                    str(item["kind"]), str(item["key"]), str(item["size"]),
                    "[green]yes[/green]" if not item["error"] else "[red]no[/red]",
                )
            self.console.print(table)
            return 0
        if action in {"issues", "failures"}:
            rows = ShowRenderer.issues(run, top=top, pdk=pdk)
            if as_json:
                print(json.dumps(rows, indent=2))
                return 0
            table = Table(title=f"Flow issues · {run.parent.name}/{run.name} · {pdk}", header_style="bold white", expand=True)
            table.add_column("Key", style="bright_cyan", no_wrap=True)
            table.add_column("State", no_wrap=True)
            table.add_column("Detail")
            table.add_column("Source")
            if not rows:
                self.console.print("[green]No non-PASS canonical reports found.[/green]")
                return 0
            for item in rows:
                table.add_row(
                    str(item["key"]), self.status(item["status"]),
                    str(item["detail"]), str(item["source"]),
                )
            self.console.print(table)
            return 0
        if action == "all":
            rows = ShowRenderer.files(run, top=top, pdk=pdk)
            if not rows:
                self.error_console.print(f"[red]no important JSON documents found under {run}[/red]")
                return 2
            ...
            for item in rows:
                if item["error"]:
                    ...
                    continue
                document = ShowRenderer.load_file(run, str(item["key"]))
            ...
            for index, item in enumerate(payload):
                if index:
                    self.console.print()
                self.console.rule(f"[bright_cyan]{item['path']}[/bright_cyan]")
                self.console.print(f"[grey70]kind:[/grey70] [white]{item['kind']}[/white]")
                if item.get("error"):
                    self.console.print(f"[red]invalid JSON: {item['error']}[/red]")
                    continue
                self.render(ShowRenderer.load_file(run, str(item["path"])))
            return 0
        try:
            document = (
                ShowRenderer.load_file(run, raw_action)
                if "/" in raw_action or raw_action.lower().endswith(".json")
                else ShowRenderer.load(run, top=top, pdk=pdk, key=action)
            )
        except (FileNotFoundError, KeyError, ValueError) as exc:
            self.error_console.print(f"[red]{exc}[/red]")
            return 2
        if as_json:
            print(json.dumps(document.data, indent=2, sort_keys=True))
            return 0
        self.console.print(f"[grey70]source:[/grey70] [white]{document.path}[/white]")
        self.render(document)
        return 0

    def requirements(self, run: Path, *, less: bool = False, as_json: bool = False) -> int:
        """Render the authoritative requirements document."""

        try:
            document = ShowRenderer.load_spec(run, "requirements")
        except (FileNotFoundError, KeyError, ValueError) as exc:
            self.error_console.print(f"[red]{exc}[/red]")
            return 2
        data = document.data
        ...
        if not isinstance(rows, list):
            self.error_console.print(f"[red]{document.path}: requirements must be a list[/red]")
            return 2
        if as_json:
        ...
        table = Table(
            ...
            header_style="bold white", expand=not less,
        )
        ...
        for item in rows:
            if not isinstance(item, Mapping):
                continue
            values = [str(item.get("id", "-")), str(item.get("status", "-"))]
            if not less:
        ...
        self.console.print(table)
        self.console.print(
            f"[grey70]requirements:[/grey70] [white]{len(rows)}[/white] · "
            f"[grey70]source:[/grey70] [white]{document.path}[/white]"
        )
        return 0

    def testplan(self, run: Path, *, less: bool = False, as_json: bool = False) -> int:
        """Render the authoritative test plan."""

        try:
            document = ShowRenderer.load_spec(run, "testplan")
        except (FileNotFoundError, KeyError, ValueError) as exc:
            self.error_console.print(f"[red]{exc}[/red]")
            return 2
        data = document.data
        ...
        if not isinstance(rows, list):
            self.error_console.print(f"[red]{document.path}: items must be a list[/red]")
            return 2
        if as_json:
            print(json.dumps(data, indent=2, sort_keys=False))
            return 0
    ...
    @staticmethod
    def _state_markup(value: object) -> str:
        token = str(value or "-").upper()
        color = "green" if token == "PASS" else "red" if token in {"FAIL", "FAILED", "INVALID"} else "orange1"
        return f"[{color}]{token}[/{color}]"

    def meta(self, run: Path, *, less: bool = False, as_json: bool = False) -> int:
        """Render release-critical metadata and its file inventory."""

        rows = ShowRenderer.meta_entries(run)
        documents = ShowRenderer.meta_documents(run)
        if as_json:
            print(json.dumps({"files": rows, "documents": documents}, indent=2))
            return 0
        if not rows:
            self.error_console.print(f"[red]no metadata found below {run / 'meta'}[/red]")
            return 2
        ...
        if not less:
            for document in documents:
                ...
                if not isinstance(data, Mapping):
                    self.error_console.print(f"[red]{path}: invalid JSON[/red]")
                    continue
                if kind == "design_intent":
                    ...
                    summary = Table(title=f"Qualification · {pdk or '-'}", header_style="bold white")
                    ...
                    summary.add_row("Status", self._state_markup(data.get("qualification_status")))
                    ...
                    if isinstance(levels, Mapping) and levels:
                        ...
                        for level, item in levels.items():
                            if isinstance(item, Mapping):
                                ...
                                level_table.add_row(f"L{level}", str(item.get("name", "-")), self._state_markup(item.get("status")), blocking)
                    ...
                    if isinstance(evidence, Mapping) and evidence:
                        ...
                        for stage, value in evidence.items():
                            evidence_table.add_row(str(stage), self._state_markup(value))
                    ...
                    if isinstance(stages, Mapping):
                        for stage, item in stages.items():
                            ...
                            table.add_row(
                                str(stage), str(len(item.get("inputs", []) or ())),
                                str(len(item.get("generated", []) or ())),
                                str(len(item.get("parents", {}) or {})),
                                self._state_markup("PASS" if path_state == "OK" else "INVALID"),
        ...
        self.console.print(table)
        self.console.print(
            f"[grey70]files:[/grey70] [white]{len(rows)}[/white] · "
            f"[grey70]root:[/grey70] [white]{run / 'meta'}[/white]"
        )
        return 0
