...
class Terminal:
    """Render command lifecycle output and normalize terminal text."""
    ...
    def print_path_label(
        ...
        *,
        ...
        use_color = Terminal.color_enabled(stream) if color is None else color
        if use_color:
    ...
    def print_script(
        ...
        *,
        ...
        use_color = Terminal.color_enabled(stream) if color is None else color
        Terminal.print_path_label("script", resolved, details=details, stream=stream, color=use_color)
        if not show_content:
            return
    ...
    def print_log(path: Path, *, stream: TextIO | None = None, color: bool | None = None) -> None:
        ...
        Terminal.print_path_label("log", path, stream=stream, color=color)
    ...
    def print_live_line(
        ...
        *,
        ...
        if structured:
            ...
            Terminal.print_label(label, text, stream=stream, color=use_color)
            return
        if use_color:
...
class LocalExecutor:
    ...
    def run(self, request: CommandRequest) -> CommandResult:
        ...
        if request.stdout is not None:
            request.stdout.parent.mkdir(parents=True, exist_ok=True)
        if request.stdout is not None and request.line_callback is not None:
            raise ValueError("CommandRequest stdout and line_callback are mutually exclusive")

        env = os.environ.copy()
        env.update(request.env)
        start = monotonic()
        with request.log.open("w", encoding="utf-8") as log:
            capture = request.stdout.open("w", encoding="utf-8") if request.stdout else None
            try:
                stdout = capture or log
                stderr = log if capture is not None else subprocess.STDOUT
                if request.detach:
                    subprocess.Popen(
                        request.argv, cwd=request.cwd, env=env, stdout=stdout, stderr=stderr,
                        text=True, start_new_session=True,
                    )
                    return CommandResult(0, request.log, monotonic() - start)
                ...
                proc = subprocess.Popen(
                    request.argv, cwd=request.cwd, env=env, stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT, text=True, bufsize=1,
                )
                assert proc.stdout is not None
                for line in proc.stdout:
                    log.write(Terminal.strip_ansi(line))
                    log.flush()
...
class SshExecutor:
    ...
    def run(self, request: CommandRequest) -> CommandResult:
        ...
        if request.stdout is not None:
            remote_stdout = self._remote(request.stdout)
            command = (
                f"mkdir -p {shlex.quote(str(remote_stdout.parent))} && "
                f"{command} > {shlex.quote(str(remote_stdout))}"
            )
        if request.detach:
            command = f"nohup sh -c {shlex.quote(command)} >/dev/null 2>&1 </dev/null &"

        start = monotonic()
        with request.log.open("w", encoding="utf-8") as log:
            if request.line_callback is None:
                proc = subprocess.run(
                    ["ssh", self.target.host, command], stdout=log,
                    stderr=subprocess.STDOUT, text=True, timeout=request.timeout_s, check=False,
                )
                returncode = proc.returncode
            else:
                proc = subprocess.Popen(
                    ["ssh", self.target.host, command], stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT, text=True, bufsize=1,
                )
                assert proc.stdout is not None
                for line in proc.stdout:
