...
class Core:
    """Compose the shared services used by every backend domain."""

    context: "BackendContext"
    runner: object | None = None
    pdk: "PdkManager" = field(init=False)
    toolchain: object = field(init=False)
    workspace: object = field(init=False)

    def __post_init__(self) -> None:
        from .flow.session import WorkspaceFlow
        ...
        self.pdk = PdkManager(self.context.project_root, self.runner)
        self.toolchain = Toolchain(self.context.project_root, self.runner)
...
class ClockConfig:
    ...
    def _domains(value: str) -> tuple[ClockDomain, ...]:
        ...
        for item in ClockConfig._items(value):
            ...
            domains.append(ClockConfig._bootstrap_domain(name, signal, reset, period_ns, polarity))
    ...
    @staticmethod
    def from_values(values: Mapping[str, object] | None = None) -> ClockConfig:
        ...
        if domains_text:
            domains = ClockConfig._domains(domains_text)
        elif requested == 1:
            period = float(raw.get("CLK_PERIOD", "10") or "10")
            domains = (ClockConfig._bootstrap_domain("core", "clk_i", "rst_ni", period),)
        else:
        ...
        relationships = ClockConfig._relationships(relationships_text, {domain.name for domain in domains})
...
class PDKRunLayout:
    ...
    @staticmethod
    def build_run_root(workspace: Path, *, run_top: str, run_id: str) -> Path:
        """Return the PDK-independent root for one logical run."""

        return workspace.expanduser().resolve() / "runs" / PDKRunLayout._slug(run_top) / PDKRunLayout._slug(run_id)

    @staticmethod
    def from_run(run_dir: Path, *, pdk: str, top: str) -> PDKRunLayout:
        """Build the canonical PDK-scoped layout below an existing run root."""

        return PDKRunLayout(run_dir.expanduser().resolve(), PDKRunLayout._slug(pdk), PDKRunLayout._slug(top))

    @staticmethod
    def from_values(project_root: Path, values: Mapping[str, str]) -> PDKRunLayout:
        ...
        return PDKRunLayout.from_run(shared, pdk=pdk, top=top)

    @staticmethod
    def settings_from_values(project_root: Path, values: Mapping[str, str]) -> dict[str, str]:
        ...
        return {
            ...
            "POST_LAYOUT_SIMDIR": str(layout.post_impl_sim_dir),
    ...
    @property
    def post_impl_sim_dir(self) -> Path:
        return self.run_root / "dv" / "functional" / "sim" / "post_impl" / self.pdk
    ...
    @property
    def post_impl_log_dir(self) -> Path:
        return self.run_root / "logs" / "dv" / "functional" / "post_impl" / self.pdk
...
class Files:
    """Create and replace FlexSoC-owned generated files safely."""
...
class RtlSources:
    """Discover SystemVerilog signatures and ordered RTL dependencies."""
    ...
    def _read_module_header_text(sv_path: Path) -> str:
        ...
        clean = "".join(out)
        return "\n".join(RtlSources._strip_line_comment(line) for line in clean.splitlines())

    @staticmethod
    def _append_parameter(line: str, target: list[tuple[str, str]]) -> None:
        """Append one parsed parameter/localparam declaration."""
    ...
    def _append_port(
        ...
        """Append one parsed input/output declaration."""

        tokens = line.strip().rstrip(",)").split()
        if tokens and tokens[0] in ("input", "output"):
            tokens = tokens[1:]
        ...
        width = tokens[-2] if len(tokens) >= 2 else 1
        name = tokens[-1].rstrip(",")
        target.append((name, width))
        if "clk_" in name:
            clocks.append(name)
        if "rst_" in name:
            resets.append(name)
    ...
    def parse_sv_signature(rtldir: str | os.PathLike[str], top: str) -> dict[str, Any]:
        ...
        lines = RtlSources._read_module_header_text(sv).splitlines()
        ...
        for raw in lines:
            ...
            if "parameter" in line and "localparam" not in line:
                RtlSources._append_parameter(line, parameters)
            elif "localparam" in line:
                RtlSources._append_parameter(line, localparams)
            elif line.startswith("input"):
                RtlSources._append_port(line, ports_in, clks, rsts)
            elif line.startswith("output"):
                RtlSources._append_port(line, ports_out, clks, rsts)
    ...
    def gather_rtl_sources(root: Path) -> list[Path]:
        ...
        return RtlSources.list_hdl_files(root, recursive=True)
    ...
    def select_used_ips_in_order(ip_candidates: list[Path], rtl_root: Path) -> list[Path]:
        ...
        rtl_files = RtlSources.gather_rtl_sources(rtl_root)

        used: list[Path] = []
        for ip in ip_candidates:
            token = ip.stem
            if any(RtlSources.file_contains_any_token(rtl, (token,)) for rtl in rtl_files):
                used.append(ip)
    ...
    def resolve_ip_dependencies(initial_used: list[Path], ip_candidates: list[Path]) -> list[Path]:
        ...
        while queue:
            cur = queue.pop(0)
            for sub in RtlSources._parse_instantiated_modules_from_sv(cur):
    ...
    def collect_nested_ip_sources(run_root: Path) -> list[Path]:
        ...
        for ip_dir in sorted(p for p in ips_dir.iterdir() if p.is_dir()):
            ...
            for filelist in lists:
                entries.extend(RtlSources._read_filelist_entries(filelist))
            if not entries:
                entries = RtlSources._read_filelist_entries(rtl_dir / "rtl_list.f")
            if not entries:
    ...
    def build_ordered_sources(
        ...
        *,
        ...
        fixed_packages = RtlSources.always_include_packages(ips_root)
        ip_candidates = RtlSources.candidate_ips_in_order(ips_root)

        used_ips_seed = RtlSources.select_used_ips_in_order(ip_candidates, rtl_root)
        used_ips = RtlSources.resolve_ip_dependencies(used_ips_seed, ip_candidates)
        ...
        all_rtl = RtlSources.gather_rtl_sources(rtl_root)
        ...
        nested_ip_sources = RtlSources.collect_nested_ip_sources(run_root)
...
class BackendContext:
    ...
    def clocks(self) -> ClockConfig:
        return ClockConfig.from_values(self.values)

    @property
    def layout(self) -> PDKRunLayout:
        return PDKRunLayout.from_values(self.project_root, self.values)

    @property
    def paths(self) -> "FlowPaths":
        """Return canonical non-technology and technology run paths."""
        return FlowPaths.from_values(self.project_root, self.workspace, self.values)
...
class PdkManager:
    ...
    def installed_root(project_root: Path, name: str, root: str | Path | None = None) -> Path:
        ...
        return PdkManager.default_root(project_root, name).expanduser().resolve()
    ...
    def _pick_lib(libs: list[Path], hint: str, kind: str) -> Path | None:
        if not libs:
            return None
        ranked = sorted(libs, key=lambda path: PdkManager._score_lib(path, hint, kind), reverse=True)
        best = ranked[0]
        # A slow/fast view is optional. Do not silently call a typical file a corner.
        if kind != "typ" and PdkManager._score_lib(best, hint, kind)[0] < 15:
            return None
        return best
    ...
    def discover_views(root: Path, name: str) -> PDKViews:
        ...
        item = PdkManager.spec(name)
        if not root.is_dir():
            return PDKViews(root, None, None, None, ())

        libs = PdkManager._files(root, (".lib",))
        hint = item.stdcell_hint.lower()
        hinted_libs = [path for path in libs if hint in path.as_posix().lower()] or libs
        models = PdkManager._stdcell_verilog_models(root, item.stdcell_hint)

        return PDKViews(
            root=root,
            liberty_typ=PdkManager._pick_lib(hinted_libs, item.stdcell_hint, "typ"),
            liberty_slow=PdkManager._pick_lib(hinted_libs, item.stdcell_hint, "slow"),
            liberty_fast=PdkManager._pick_lib(hinted_libs, item.stdcell_hint, "fast"),
            verilog_models=models,
        )

    @staticmethod
    def settings(project_root: Path, name: str, root: str | Path | None = None) -> dict[str, str]:
        """Return PDK-dependent FlexSoC settings for discovered views."""

        item = PdkManager.spec(name)
        install = PdkManager.installed_root(project_root, item.name, root)
        views = PdkManager.discover_views(install, item.name)
        values = {
            "PDK": item.name,
        ...
        adapter = PdkManager.formal_adapter_path(project_root, item.name)
    ...
    def _fetch_formal_adapter(project_root: Path, item: PDKSpec, install: Path) -> Path | None:
        ...
        path = PdkManager.formal_adapter_path(project_root, item.name)
        if path is None:
            return None
        try:
        ...
        temp.chmod(0o755)
        temp.replace(path)
        metadata = PdkManager.fetch_metadata(install)
        metadata["formal_adapter"] = {
            "path": str(path),
            "source_url": item.formal_adapter_url,
        }
        PdkManager._write_fetch_metadata(install, metadata)
        return path
    ...
    def _write_fetch_metadata(root: Path, data: dict[str, object]) -> None:
        path = PdkManager._fetch_metadata_path(root)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
    ...
    @staticmethod
    def _ciel_executable(*, runner=None, on: str = "local") -> str:
        """Return the Ciel command for the selected execution target."""

        if runner is not None and on in runner.targets and runner.targets[on].kind != "local":
            return "ciel"
    ...
    @staticmethod
    def _run_ciel(
        command: list[str], *, project_root: Path, runner=None, on: str = "local",
        outputs: tuple[Path, ...] = (),
    ) -> str:
        """Run Ciel through ToolRunner and return its transcript."""

        from .runtime.execution import CommandRequest, ToolRunner

        active_runner = runner or ToolRunner(project_root=project_root)
        action = command[1] if len(command) > 1 else "command"
        log = project_root / ".flexsoc" / "logs" / "pdk" / f"ciel-{action}.log"
        result = active_runner.run(
            CommandRequest(tuple(command), project_root, {}, log, outputs=outputs), on=on
        )
    ...
    @staticmethod
    def _latest_ciel_version(
        executable: str, pdk_root: Path, family: str, *, project_root: Path, runner=None,
        on: str = "local",
    ) -> str:
        ...
        text = PdkManager._run_ciel(command, project_root=project_root, runner=runner, on=on)
        matches = re.findall(r"\b([0-9a-f]{40})\b", text, flags=re.IGNORECASE)
        if not matches:
            raise ValueError(
    ...
    def _fetch_with_ciel(
        ...
        *,
        ...
        executable = PdkManager._ciel_executable(runner=runner, on=on)
        pdk_root = project_root / ".flexsoc" / "pdks"
        dest = PdkManager.default_root(project_root, item.name)
        if dest.exists() or dest.is_symlink():
            if not force:
                raise ValueError(f"PDK path already exists: {dest}; use --force to replace it")
            PdkManager._remove_path(dest)
        pdk_root.mkdir(parents=True, exist_ok=True)
        selected = version or PdkManager._latest_ciel_version(
            executable, pdk_root, item.name, project_root=project_root, runner=runner, on=on
        )
        ...
        PdkManager._run_ciel(
            command, project_root=project_root, runner=runner, on=on, outputs=(pdk_root,)
        )
        ...
        PdkManager._write_fetch_metadata(
            dest,
            {
                "name": item.name,
                "provider": "ciel",
    ...
    def _fetch(
        ...
        *,
        ...
        if item.fetch_provider == "ciel":
            dest = PdkManager._fetch_with_ciel(
                project_root, item, force=force, version=version, runner=runner, on=on
            )
            PdkManager._fetch_formal_adapter(project_root, item, dest)
            return dest
        ...
        active_runner = runner or ToolRunner(project_root=project_root)
        log = project_root / ".flexsoc" / "logs" / "pdk" / f"git-{item.name}.log"
        result = active_runner.run(
            CommandRequest(tuple(command), project_root, {}, log, outputs=(dest,)), on=on
        )
        if result.returncode != 0:
            raise ValueError(f"Git PDK fetch failed ({result.returncode}); log: {log}")
        PdkManager._write_fetch_metadata(
            dest,
            {
                "name": item.name,
    ...
    def describe(project_root: Path, name: str, root: str | Path | None = None) -> dict[str, object]:
        ...
        return {
            **asdict(item),
            "views": PdkManager.discover_views(install, item.name).to_dict(),
            "fetch": PdkManager.fetch_metadata(install),
    ...
    @staticmethod
    def catalog(project_root: Path) -> list[dict[str, object]]:
        """Return every catalogue row with local install status."""

        return [PdkManager.describe(project_root, name) for name in PDKS]
    ...
    def list(self) -> list[dict[str, object]]:
        return PdkManager.catalog(self.project_root)

    def info(self, name: str, root: str | Path | None = None) -> dict[str, object]:
        return PdkManager.describe(self.project_root, name, root)

    def fetch(
        self, name: str, *, force: bool = False, version: str | None = None,
        on: str = "local",
    ) -> Path:
        return PdkManager._fetch(
            self.project_root, name, force=force, version=version, runner=self.runner, on=on
        )

    def use(self, name: str, root: str | Path | None = None) -> dict[str, str]:
        return PdkManager.settings(self.project_root, name, root)

    def views(self, name: str, root: str | Path | None = None) -> PDKViews:
        canonical = PdkManager.normalize_name(name)
        path = Path(root).expanduser().resolve() if root else PdkManager.default_root(self.project_root, canonical)
        return PdkManager.discover_views(path, canonical)
...
class FlowPaths:
    ...
    @staticmethod
    def from_values(project_root: Path, workspace: Path, values: Mapping[str, str]) -> FlowPaths:
