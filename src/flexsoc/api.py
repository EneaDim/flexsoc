...
class FlexSoCTarget:
    ...
    def _execute_target(self, target: str) -> object:
        ...
        if target == "hjson":
            return b.design.regs.init_hjson(top, interface, p.csr, force=force, clocks=self.context.clocks)
        if target == "reg":
            return b.design.regs.setup_rtl(top, p.csr, p.rtl, regmap=v.get("REGMAP"), on=self.on)
        if target == "doc":
            return b.design.regs.setup_docs(top, p.csr, p.doc, regmap=v.get("REGMAP"), on=self.on)
        if target == "systemrdl":
            return b.design.regs.setup_systemrdl(
                top, p.csr, p.csr_systemrdl,
                regmap=v.get("REGMAP"), on=self.on,
            )
        if target == "ipxact":
            return b.package.export_ipxact(
                top=top, csr_dir=p.csr, rtl_dir=p.rtl, output=p.run / "component.xml",
                vendor=v.get("IPXACT_VENDOR", "flexsoc"),
                library=v.get("IPXACT_LIBRARY", "ip"),
                version=v.get("IPXACT_VERSION", "1.0.0"),
            )
        if target == "driver":
            return b.design.regs.setup_driver(p.csr / f"{top}.hjson", p.drivers, base_address=v.get("BASE_ADDRESS", "0x0"))
        if target == "regmap_py":
            return b.design.regs.setup_regmap_py(top, p.csr, p.model, force=force, refresh_tests=True, clocks=self.context.clocks)
        if target == "rtl_stub":
            hjson = p.csr / f"{top}.hjson"
        ...
        if target == "model.setup":
            return b.design.model.flow(top, p.csr, p.model, p.rtl, force=force, clocks=self.context.clocks)
        ...
        if target in {"cocotb", "cocotb_tests"}:
            if target == "cocotb_tests":
                return b.dv.functional.run_regression(top=top, test_root=p.tests, tb_dir=p.tb, sim_dir=p.sim / "rtl", common_filelist=p.rtl_common, ip_filelist=p.rtl_ip, rtl_sources=self._rtl_sources(), compiler=v.get("COMPILER","verilator"), backends=("cocotb",), seed=int(v.get("SEED","1")), reset_settle_cycles=int(v.get("RESET_SETTLE_CYCLES","8")), log_dir=p.logs / "dv" / "functional", on=self.on)
            return b.dv.functional.run_cocotb(top=top, test_root=p.tests, tb_dir=p.tb, rtl_sources=self._rtl_sources(), test_name=v.get("TEST_NAME","smoke"), simulator=v.get("COMPILER","verilator"), seed=int(v.get("SEED","1")), reset_settle_cycles=int(v.get("RESET_SETTLE_CYCLES","8")), waves=self._bool(v.get("COCOTB_WAVES"), True), log=p.logs / "dv" / "functional" / f"{top}_cocotb_{v.get('TEST_NAME','smoke')}.log", on=self.on)
