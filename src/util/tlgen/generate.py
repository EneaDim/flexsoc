...
def generate(xbar: Xbar, library_name: str = "ip") -> List[Tuple[str, Any]]:
    ...
    xbar_rtl_tpl = Template(
        filename=str(files('tlgen').joinpath('xbar.rtl.sv.tpl')))
    xbar_pkg_tpl = Template(
        filename=str(files('tlgen').joinpath('xbar.pkg.sv.tpl')))
    xbar_core_tpl = Template(
        filename=str(files('tlgen').joinpath('xbar.core.tpl')))
    xbar_hjson_tpl = Template(
        filename=str(files('tlgen').joinpath('xbar.hjson.tpl')))
    try:
