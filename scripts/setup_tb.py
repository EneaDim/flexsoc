...
try:
    ...
    if compiler == 'verilator' and flag_reg_pkg and itf == 'reg_iface':
        with open(os.path.join(tb_path, 'reg_if.sv'), 'w+') as f:
            ...
            mystr += '  parameter int unsigned AW  = 4,\n'
            mystr += '  parameter int unsigned DW  = 32,\n'
            mystr += '  parameter int unsigned DBW = DW/8,\n'
            mystr += '  parameter type reg_req_t = struct packed {\n'
            mystr += '    logic           valid;\n'
            mystr += '    logic           write;\n'
            mystr += '    logic [AW-1:0]  addr;\n'
            mystr += '    logic [DW-1:0]  wdata;\n'
            mystr += '    logic [DBW-1:0] wstrb;\n'
            mystr += '  },\n'
            mystr += '  parameter type reg_rsp_t = struct packed {\n'
            mystr += '    logic [DW-1:0]  rdata;\n'
            mystr += '    logic           error;\n'
            mystr += '    logic           ready;\n'
            mystr += '  }\n'
            mystr += ')(\n'
        ...
        with open(os.path.join(tb_path, 'reg_utils.sv'), 'w+') as f:
            ...
            mystr += '  virtual reg_if drv_if;\n\n'
            mystr += '  function new(virtual reg_if drv_if);\n'
            mystr += '    this.drv_if = drv_if;\n'
            mystr += '  endfunction\n\n'
            mystr += '  task automatic write(input  logic ['+str(top)+'_reg_pkg::AW-1:0]  addr,\n'
            mystr += '                       input  logic ['+str(top)+'_reg_pkg::DW-1:0]  data,\n'
            mystr += '                       input  logic ['+str(top)+'_reg_pkg::DBW-1:0] strb = {'+str(top)+'_reg_pkg::DBW{1\'b1}});\n'
            mystr += '    drv_if.req.valid <= 1;\n'
            mystr += '    drv_if.req.write <= 1;\n'
            mystr += '    drv_if.req.addr  <= addr;\n'
            mystr += '    drv_if.req.wdata <= data;\n'
            mystr += '    drv_if.req.wstrb <= strb;\n'
            mystr += '    @(posedge drv_if.clk_i);\n'
            mystr += '    wait (drv_if.rsp.ready);\n'
            mystr += '    drv_if.req.valid <= 0;\n'
            mystr += '  endtask\n\n'
            mystr += '  task automatic read(input  logic ['+str(top)+'_reg_pkg::AW-1:0]  addr,\n'
            mystr += '                       output logic ['+str(top)+'_reg_pkg::DW-1:0]  data);\n'
            mystr += '    drv_if.req.valid <= 1;\n'
            mystr += '    drv_if.req.write <= 0;\n'
            mystr += '    drv_if.req.addr  <= addr;\n'
            mystr += '    drv_if.req.wdata <= \'0;\n'
            mystr += '    drv_if.req.wstrb <= \'0;\n'
            mystr += '    @(posedge drv_if.clk_i);\n'
            mystr += '    wait (drv_if.rsp.ready);\n'
    ...
    with open(os.path.join(base_path, top + '_tb.sv'), 'w+') as f:
        ...
        if compiler == 'verilator':
            if flag_reg_pkg and itf == 'tlul':
                mystr += '    // tlul_utils usage example (commented)\n'
                mystr += '    // tl_utils_inst = new(tl_if);\n'
                mystr += '    // #(CLK_PERIOD*10);\n'
                mystr += '    // tl_utils_inst.tlul_write(32\'h0, 32\'h1, 4\'h0);\n'
                mystr += '    // #(CLK_PERIOD*10);\n'
            if flag_reg_pkg and itf == 'reg_iface':
                mystr += '    // reg_utils usage example (commented)\n'
                mystr += '    // reg_utils_inst = new(regif);\n'
                mystr += '    // logic [regif.DW-1:0] rdata;\n'
                mystr += '    // #(CLK_PERIOD*10);\n'
                mystr += '    // reg_utils_inst.write(\'h0, 32\'hDEADBEEF);\n'
                mystr += '    // #(CLK_PERIOD*10);\n'
